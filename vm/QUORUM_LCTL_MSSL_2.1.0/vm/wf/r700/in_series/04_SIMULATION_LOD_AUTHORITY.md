...
Replace binary loaded/unloaded behavior with continuous simulation-detail states so remote entities remain alive at reduced computational fidelity.
...
- **RCPW-04-R04:** LOD decisions combine distance with visibility, causality, narrative gravity, threat, and resource budgets.
- **RCPW-04-R05:** Resource exhaustion degrades fidelity without silently erasing authoritative state.
- **RCPW-04-R06:** LOD decisions are replayable from recorded deterministic inputs.

- **RCPW-04-R07:** LOD scheduling has starvation bounds so causally important remote entities cannot be indefinitely deferred.
- **RCPW-04-R08:** Update cadence changes preserve time integration and do not create energy/state jumps when promoted.
- **RCPW-04-R09:** Budget governor decisions are observable, replayable, and attributed to CPU/GPU/memory/I/O pressure.
- **RCPW-04-R10:** Mass promotion storms are rate-limited without erasing unresolved authoritative obligations.

- **RCPW-04-R11:** Cross-LOD simulation preserves event barriers: events requiring higher fidelity suspend, escalate, or resolve through an explicitly qualified abstract model.
...
- **RCPW-04-R16:** Cross-subsystem LOD combinations are validated so incompatible fidelity pairs cannot create causally impossible results.
- **RCPW-04-R17:** LOD promotion scheduling reserves capacity for interaction-critical entities, event barriers, and recovery work before cosmetic/background promotions.
...
- **RCPW-04-R19:** LOD telemetry records approximation class, error budget, deferred obligations, promotion debt, and last full-fidelity checkpoint.

- **RCPW-04-R20:** Simulation LOD scheduling supports deterministic parallel work assignment while canonical commit order remains stable and replayable.
- **RCPW-04-R21:** Subsystem LOD jobs declare read/write sets or equivalent conflict domains so unsafe parallel updates cannot silently race.
- **RCPW-04-R22:** LOD work stealing or dynamic scheduling may change execution placement but not canonical results in deterministic qualification profiles.
- **RCPW-04-R23:** Approximation debt and promotion debt survive worker restart, checkpoint/recovery, and partition migration.
- **RCPW-04-R24:** LOD qualification compares single-worker and multi-worker canonical checkpoint digests for semantic equivalence.
...
1. Define L0 state-only, L1 statistical, L2 strategic, L3 navigational, L4 kinematic, L5 physical, L6 visual, L7 interactive, and L8 reference-authority tiers.
2. Define per-class payloads and invariants for each tier.
3. Define promotion/demotion conditions, hysteresis, minimum residency time, update rate, and resource quotas.
4. Implement a deterministic priority score incorporating distance, visibility, causal significance, narrative gravity, threat, interaction forecast, and budget.
5. Implement adjacent-tier conversion functions and prove round-trip state preservation.
6. Add overload policy that sheds derived fidelity before authoritative state.
7. Stress millions of low-fidelity entities with burst promotion around a fast-moving reference point.
8. Replay identical LOD decision traces and verify equivalent authoritative outcomes and bounded performance.
...
- `SIMULATION_LOD.mssl`
- `simulation_lod.lctlc`
- `LOD_LADDER.md`
- `LOD_POLICY.json`
- `LOD_BUDGETS.json`
- `LOD_REPLAY_EVIDENCE.json`
- `RCPW-04_requirements.json`
- `RCPW-04_tests.log`
- `RCPW-04_audit.md`
- `RCPW-04_manifest.sha256`
...
|---|---|---|---|
| RCPW-04-R01 | Every persistent entity class has a declared simulation representation ladder. | NOT_STARTED | — |
| RCPW-04-R02 | Promotion/demotion preserves identity, causal obligations, and gameplay-significant state. | NOT_STARTED | — |
| RCPW-04-R03 | Hysteresis prevents transition thrashing. | NOT_STARTED | — |
| RCPW-04-R04 | LOD decisions combine distance with visibility, causality, narrative gravity, threat, and resource budgets. | NOT_STARTED | — |
| RCPW-04-R05 | Resource exhaustion degrades fidelity without silently erasing authoritative state. | NOT_STARTED | — |
| RCPW-04-R06 | LOD decisions are replayable from recorded deterministic inputs. | NOT_STARTED | — |
| RCPW-04-R07 | LOD scheduling has starvation bounds so causally important remote entities cannot be indefinitely deferred. | NOT_STARTED | — |
| RCPW-04-R08 | Update cadence changes preserve time integration and do not create energy/state jumps when promoted. | NOT_STARTED | — |
| RCPW-04-R09 | Budget governor decisions are observable, replayable, and attributed to CPU/GPU/memory/I/O pressure. | NOT_STARTED | — |
| RCPW-04-R10 | Mass promotion storms are rate-limited without erasing unresolved authoritative obligations. | NOT_STARTED | — |

| RCPW-04-R11 | Cross-LOD simulation preserves event barriers: events requiring higher fidelity suspend, escalate, or resolve through an explicitly qualified abstract model. | NOT_STARTED | — |
| RCPW-04-R12 | Remote AI schedule, intent, movement, and social-state integration account for elapsed canonical time when update cadence changes. | NOT_STARTED | — |
| RCPW-04-R13 | Promotion debt is tracked so repeatedly deferred high-value entities receive bounded service without creating an uncontrolled promotion storm. | NOT_STARTED | — |
| RCPW-04-R14 | LOD approximation error is measured per subsystem and incorporated into qualification rather than treated as visually acceptable by default. | NOT_STARTED | — |

| RCPW-04-R15 | Simulation LOD authority defines subsystem-specific fidelity contracts for AI, ecology, economy, weather, navigation, combat, transport, and narrative events. | NOT_STARTED | — |
| RCPW-04-R16 | Cross-subsystem LOD combinations are validated so incompatible fidelity pairs cannot create causally impossible results. | NOT_STARTED | — |
| RCPW-04-R17 | LOD promotion scheduling reserves capacity for interaction-critical entities, event barriers, and recovery work before cosmetic/background promotions. | NOT_STARTED | — |
| RCPW-04-R18 | Approximation debt is accumulated and repaid when an entity or region returns to higher fidelity, with bounded reconciliation work. | NOT_STARTED | — |
| RCPW-04-R19 | LOD telemetry records approximation class, error budget, deferred obligations, promotion debt, and last full-fidelity checkpoint. | NOT_STARTED | — |

| RCPW-04-R20 | Simulation LOD scheduling supports deterministic parallel work assignment while canonical commit order remains stable and replayable. | NOT_STARTED | — |
| RCPW-04-R21 | Subsystem LOD jobs declare read/write sets or equivalent conflict domains so unsafe parallel updates cannot silently race. | NOT_STARTED | — |
| RCPW-04-R22 | LOD work stealing or dynamic scheduling may change execution placement but not canonical results in deterministic qualification profiles. | NOT_STARTED | — |
| RCPW-04-R23 | Approximation debt and promotion debt survive worker restart, checkpoint/recovery, and partition migration. | NOT_STARTED | — |
| RCPW-04-R24 | LOD qualification compares single-worker and multi-worker canonical checkpoint digests for semantic equivalence. | NOT_STARTED | — |

| RCPW-04-R25 | Simulation LOD includes ultra-long-horizon demographic, institutional, ecological, economic, and infrastructure representations for generational world evolution. | NOT_STARTED | — |
| RCPW-04-R26 | Long-horizon abstraction cannot resolve events whose outcome depends on unknown player-local information or unresolved high-fidelity barriers. | NOT_STARTED | — |
| RCPW-04-R27 | LOD models expose temporal validity windows so coarse long-horizon predictions are invalidated/recomputed when canonical shocks exceed assumptions. | NOT_STARTED | — |
| RCPW-04-R28 | Approximation debt includes historical uncertainty and is reduced through re-simulation or evidence-backed reconstruction when a region becomes relevant again. | NOT_STARTED | — |
| RCPW-04-R29 | Qualification compares short-step and long-horizon simulations over selected reference regions to quantify drift and acceptable semantic error. | NOT_STARTED | — |
