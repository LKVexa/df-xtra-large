...
Create an append-only world event ledger that records why the world changed, not merely what its current values are.
...
- **RCPW-06-R03:** Replay from a trusted snapshot produces equivalent authoritative world state.
- **RCPW-06-R04:** Conflicting concurrent mutations are detected and resolved by an explicit policy.
- **RCPW-06-R05:** Ledger integrity is hash-bound and partial/corrupt tails fail closed.
- **RCPW-06-R06:** Ephemeral telemetry is separated from durable causal history.

- **RCPW-06-R07:** Ledger ordering semantics are explicit for same-tick and cross-thread events.
- **RCPW-06-R08:** Hash-chain or equivalent tamper-evidence covers segment boundaries, snapshots, and replay cut points.
- **RCPW-06-R09:** Compaction/aggregation cannot sever causal parents required by live unresolved obligations.
- **RCPW-06-R10:** Ledger replay divergence emits a minimal deterministic counterexample rather than a generic failure.

- **RCPW-06-R11:** Causal graphs detect or explicitly model cycles and prevent accidental recursive event amplification.
- **RCPW-06-R12:** Durable events are idempotent under replay and repeated recovery.
- **RCPW-06-R13:** Ledger compaction preserves unresolved causes, obligations, ownership transfers, and historical evidence needed for later materialization.
...
- **RCPW-06-R15:** Causal ledger events use stable event-class schemas with forward-compatible unknown-field handling and explicit rejection of incompatible semantics.
- **RCPW-06-R16:** Ledger commit is coordinated with canonical-state commit so no durable event claims a mutation that failed to commit.
- **RCPW-06-R17:** Remote abstract resolutions record the model/version and fidelity class that produced the outcome.
- **RCPW-06-R18:** Ledger retention can preserve compact causal proofs for long-lived world consequences after detailed telemetry is discarded.
- **RCPW-06-R19:** Causal-debug tooling can bisect replay divergence to the first canonical revision/event at which state digests differ.

- **RCPW-06-R20:** Causal ledger supports partition-local staging with deterministic global commit/merge semantics for causally interacting events.
...
- **RCPW-06-R23:** Ledger compaction preserves cross-partition causal links and migration provenance.
- **RCPW-06-R24:** Replay verification can reproduce a global causal cut from multiple partition segments and detect missing or duplicated commits.

- **RCPW-06-R25:** Causal ledger supports era-spanning causal summaries that preserve proof chains for long-lived institutions, ownership, reputation, settlement change, and inherited obligations.
...
- **RCPW-06-R29:** Qualification reconstructs present-day world facts from old checkpoints plus compacted multi-era ledger segments and verifies semantic digests.

## Stage-Specific Engineering Workflow

1. Define event schema, deterministic event IDs, causal parent links, vector/sequence ordering policy, and transaction boundaries.
2. Define which world events are durable versus ephemeral telemetry.
3. Implement append, commit, checksum/hash chain, recovery, truncation detection, and branch comparison.
4. Implement replay, rollback-to-checkpoint, causal trace queries, and region/entity/time filters.
5. Define deterministic conflict resolution or explicit serialization for simultaneous writes.
6. Integrate event emission with ecology, economy, narrative, combat, destruction, ownership, transport, and persistence.
7. Inject torn writes, stale snapshots, hash mismatches, duplicate events, reordered events, and conflicting mutations.
8. Prove reconstruction of selected world intervals from snapshot plus ledger only.
...
- `causal_ledger.lctlc`
- `EVENT_SCHEMA.json`
- `LEDGER_INTEGRITY_SPEC.md`
- `REPLAY_PROTOCOL.md`
- `CAUSAL_REPLAY_EVIDENCE.json`
- `RCPW-06_requirements.json`
- `RCPW-06_tests.log`
- `RCPW-06_audit.md`
- `RCPW-06_manifest.sha256`
...
|---|---|---|---|
| RCPW-06-R01 | Durable world mutations emit causally linked events with deterministic IDs. | NOT_STARTED | — |
| RCPW-06-R02 | Events record time, actor/entity, canonical location, cause, pre-state/post-state references, authority level, seed, and dependencies. | NOT_STARTED | — |
| RCPW-06-R03 | Replay from a trusted snapshot produces equivalent authoritative world state. | NOT_STARTED | — |
| RCPW-06-R04 | Conflicting concurrent mutations are detected and resolved by an explicit policy. | NOT_STARTED | — |
| RCPW-06-R05 | Ledger integrity is hash-bound and partial/corrupt tails fail closed. | NOT_STARTED | — |
| RCPW-06-R06 | Ephemeral telemetry is separated from durable causal history. | NOT_STARTED | — |
| RCPW-06-R07 | Ledger ordering semantics are explicit for same-tick and cross-thread events. | NOT_STARTED | — |
| RCPW-06-R08 | Hash-chain or equivalent tamper-evidence covers segment boundaries, snapshots, and replay cut points. | NOT_STARTED | — |
| RCPW-06-R09 | Compaction/aggregation cannot sever causal parents required by live unresolved obligations. | NOT_STARTED | — |
| RCPW-06-R10 | Ledger replay divergence emits a minimal deterministic counterexample rather than a generic failure. | NOT_STARTED | — |

| RCPW-06-R11 | Causal graphs detect or explicitly model cycles and prevent accidental recursive event amplification. | NOT_STARTED | — |
| RCPW-06-R12 | Durable events are idempotent under replay and repeated recovery. | NOT_STARTED | — |
| RCPW-06-R13 | Ledger compaction preserves unresolved causes, obligations, ownership transfers, and historical evidence needed for later materialization. | NOT_STARTED | — |
| RCPW-06-R14 | Debugging can produce a minimal causal slice showing why a selected entity or region reached its current authoritative state. | NOT_STARTED | — |

| RCPW-06-R15 | Causal ledger events use stable event-class schemas with forward-compatible unknown-field handling and explicit rejection of incompatible semantics. | NOT_STARTED | — |
| RCPW-06-R16 | Ledger commit is coordinated with canonical-state commit so no durable event claims a mutation that failed to commit. | NOT_STARTED | — |
| RCPW-06-R17 | Remote abstract resolutions record the model/version and fidelity class that produced the outcome. | NOT_STARTED | — |
| RCPW-06-R18 | Ledger retention can preserve compact causal proofs for long-lived world consequences after detailed telemetry is discarded. | NOT_STARTED | — |
| RCPW-06-R19 | Causal-debug tooling can bisect replay divergence to the first canonical revision/event at which state digests differ. | NOT_STARTED | — |

| RCPW-06-R20 | Causal ledger supports partition-local staging with deterministic global commit/merge semantics for causally interacting events. | NOT_STARTED | — |
| RCPW-06-R21 | Event sequence/causal ordering remains reconstructible even when simulation work executes concurrently. | NOT_STARTED | — |
| RCPW-06-R22 | Ledger replication or redundant persistence, where configured, uses explicit quorum/commit semantics and never treats an uncommitted replica as canonical. | NOT_STARTED | — |
| RCPW-06-R23 | Ledger compaction preserves cross-partition causal links and migration provenance. | NOT_STARTED | — |
| RCPW-06-R24 | Replay verification can reproduce a global causal cut from multiple partition segments and detect missing or duplicated commits. | NOT_STARTED | — |

| RCPW-06-R25 | Causal ledger supports era-spanning causal summaries that preserve proof chains for long-lived institutions, ownership, reputation, settlement change, and inherited obligations. | NOT_STARTED | — |
| RCPW-06-R26 | Historical compaction records uncertainty and aggregation class rather than presenting compressed history as exact event detail. | NOT_STARTED | — |
| RCPW-06-R27 | Counterfactual/debug branches are isolated from canonical history and cannot leak mutations into the authoritative timeline. | NOT_STARTED | — |
| RCPW-06-R28 | Causal queries can identify both direct causes and historically inherited causes across generational/organizational succession. | NOT_STARTED | — |
| RCPW-06-R29 | Qualification reconstructs present-day world facts from old checkpoints plus compacted multi-era ledger segments and verifies semantic digests. | NOT_STARTED | — |
