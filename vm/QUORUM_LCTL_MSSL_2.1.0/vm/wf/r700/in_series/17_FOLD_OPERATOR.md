...
Formalize the transformation that maps canonical persistent reality into a finite reference-relative folded projection.
...
- **RCPW-17-R02:** Translation, rebasing, compactification, shell assignment, representation selection, and projection are separately testable.
...
- **RCPW-17-R07:** Fold operator parameter changes are versioned and recorded so replay uses the exact historical mapping.
...
- **RCPW-17-R11:** The fold operator has machine-checkable algebraic/property tests for near-field identity, monotonic radial order, bounded projection, canonical target uniqueness, and deterministic re-reference.
- **RCPW-17-R12:** Fold derivatives or equivalent local distortion measures are bounded per shell; violation triggers explicit fallback or safe mode.
- **RCPW-17-R13:** Operator composition and parameter versions are written into replay evidence whenever they affect visible/reference state.
- **RCPW-17-R14:** Fold failure handling can revert to a safe non-folded or reduced-scope projection without corrupting canonical state.
...
- **RCPW-17-R16:** Fold policy is compiled/versioned as data so identical world-package plus fold-policy versions reproduce the same reference projection.
- **RCPW-17-R17:** Operator fallback states are explicit: normal fold, conservative fold, widened near-field, reduced horizon, and safe non-folded local mode.
- **RCPW-17-R18:** Operator performance is bounded with declared per-frame/per-tick cost independent from world canonical population size except through active representation sets.
...
- **RCPW-17-R21:** Parallel fold evaluation merges representation outputs deterministically for seam, landmark, horizon, and interaction-target decisions.
- **RCPW-17-R22:** Canonical targeting never depends on which worker produced the visible representation.
- **RCPW-17-R23:** Safe-mode fold fallback is globally coordinated for the affected reference view and is replay-visible.
...
- **RCPW-17-R25:** Fold operator exposes stable behavior under canonical world-extent growth; adding distant regions cannot perturb existing near-field projection beyond declared policy bounds.
- **RCPW-17-R26:** Operator policy explicitly handles undefined frontier, generated frontier, archived world, active remote world, and local materialized world as distinct semantic classes.
- **RCPW-17-R27:** Fold summaries at the horizon may aggregate regions but preserve deterministic canonical region membership and de-duplication.
- **RCPW-17-R28:** Historical route/landmark changes update folded summaries transactionally at canonical event boundaries.
- **RCPW-17-R29:** Qualification includes incremental world growth, multiple historical eras, summary regeneration, policy upgrade/rollback, and deterministic replay.

## Stage-Specific Engineering Workflow

1. Define F_P: W_C -> W_P with reference P, fold parameters, shell policy, simulation state, and time context.
2. Decompose F_P into canonical lookup, relative transform, origin rebase, radial/anisotropic compactification, shell classifier, representation selector, and renderer/embodiment projection.
3. Specify each operator's domain, codomain, units, precision, continuity class, reversibility, and loss model.
4. Define inverse/partial inverse functions used for interaction, picking, navigation, and canonical mutation targeting.
5. Integrate optional 8S/Penteract scoring as a policy input only through the formally mapped adapter from Stage 16.
6. Add property-based tests for monotonicity, boundedness, identity in near field, stable ordering, deterministic re-reference, and valid canonical target recovery.
7. Test moving canonical objects while P moves independently.
8. Publish singularity, edge-condition, overflow, precision, and unsupported-domain behavior.
...
- `FOLD_OPERATOR.mssl`
- `fold_operator.lctlc`
- `FOLD_OPERATOR_FORMALISM.md`
- `OPERATOR_DECOMPOSITION.json`
- `PROPERTY_TESTS.json`
- `FOLD_OPERATOR_PROOF_REPORT.md`
- `RCPW-17_requirements.json`
- `RCPW-17_tests.log`
- `RCPW-17_audit.md`
- `RCPW-17_manifest.sha256`
...
|---|---|---|---|
| RCPW-17-R01 | F_P(W_C)=W_P has explicit domain, codomain, parameters, and reference authority. | NOT_STARTED | — |
| RCPW-17-R02 | Translation, rebasing, compactification, shell assignment, representation selection, and projection are separately testable. | NOT_STARTED | — |
| RCPW-17-R03 | Visible-domain components are continuous and bounded. | NOT_STARTED | — |
| RCPW-17-R04 | Lossy presentation components are explicitly identified and never treated as canonical inverse authority. | NOT_STARTED | — |
| RCPW-17-R05 | Changing reference points does not alter canonical world identity. | NOT_STARTED | — |
| RCPW-17-R06 | Singularities and unsupported domains are explicit and fail closed. | NOT_STARTED | — |
| RCPW-17-R07 | Fold operator parameter changes are versioned and recorded so replay uses the exact historical mapping. | NOT_STARTED | — |
| RCPW-17-R08 | Interaction targeting round-trips to one canonical entity or fails explicitly; ambiguous aliases are forbidden. | NOT_STARTED | — |
| RCPW-17-R09 | The near-field identity transform has measurable error bounds approaching numerical precision. | NOT_STARTED | — |
| RCPW-17-R10 | Operator composition order is fixed and verified; reordering operators requires a versioned migration. | NOT_STARTED | — |

| RCPW-17-R11 | The fold operator has machine-checkable algebraic/property tests for near-field identity, monotonic radial order, bounded projection, canonical target uniqueness, and deterministic re-reference. | NOT_STARTED | — |
| RCPW-17-R12 | Fold derivatives or equivalent local distortion measures are bounded per shell; violation triggers explicit fallback or safe mode. | NOT_STARTED | — |
| RCPW-17-R13 | Operator composition and parameter versions are written into replay evidence whenever they affect visible/reference state. | NOT_STARTED | — |
| RCPW-17-R14 | Fold failure handling can revert to a safe non-folded or reduced-scope projection without corrupting canonical state. | NOT_STARTED | — |

| RCPW-17-R15 | Fold-operator implementation exposes a canonical targeting service shared by interaction, navigation, audio landmarks, and debug tooling. | NOT_STARTED | — |
| RCPW-17-R16 | Fold policy is compiled/versioned as data so identical world-package plus fold-policy versions reproduce the same reference projection. | NOT_STARTED | — |
| RCPW-17-R17 | Operator fallback states are explicit: normal fold, conservative fold, widened near-field, reduced horizon, and safe non-folded local mode. | NOT_STARTED | — |
| RCPW-17-R18 | Operator performance is bounded with declared per-frame/per-tick cost independent from world canonical population size except through active representation sets. | NOT_STARTED | — |
| RCPW-17-R19 | Qualification includes randomized reference paths, moving landmarks, topology changes, portal crossings, and extreme entity density near fold horizons. | NOT_STARTED | — |

| RCPW-17-R20 | Fold-operator policy is distributed as immutable versioned configuration so all workers evaluating one canonical epoch use the same operator version. | NOT_STARTED | — |
| RCPW-17-R21 | Parallel fold evaluation merges representation outputs deterministically for seam, landmark, horizon, and interaction-target decisions. | NOT_STARTED | — |
| RCPW-17-R22 | Canonical targeting never depends on which worker produced the visible representation. | NOT_STARTED | — |
| RCPW-17-R23 | Safe-mode fold fallback is globally coordinated for the affected reference view and is replay-visible. | NOT_STARTED | — |
| RCPW-17-R24 | Operator qualification includes heterogeneous workload placement, worker restart, policy rollback, and deterministic seam reconstruction. | NOT_STARTED | — |

| RCPW-17-R25 | Fold operator exposes stable behavior under canonical world-extent growth; adding distant regions cannot perturb existing near-field projection beyond declared policy bounds. | NOT_STARTED | — |
| RCPW-17-R26 | Operator policy explicitly handles undefined frontier, generated frontier, archived world, active remote world, and local materialized world as distinct semantic classes. | NOT_STARTED | — |
| RCPW-17-R27 | Fold summaries at the horizon may aggregate regions but preserve deterministic canonical region membership and de-duplication. | NOT_STARTED | — |
| RCPW-17-R28 | Historical route/landmark changes update folded summaries transactionally at canonical event boundaries. | NOT_STARTED | — |
| RCPW-17-R29 | Qualification includes incremental world growth, multiple historical eras, summary regeneration, policy upgrade/rollback, and deterministic replay. | NOT_STARTED | — |
