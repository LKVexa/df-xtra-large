...
Guarantee stable unfolding ahead of the reference point and folding behind it during rapid movement and instantaneous relocation.
...
- **RCPW-15-R01:** Forward materialization lead time is predicted from motion and route/camera intent.
- **RCPW-15-R02:** Rear folding preserves persistent state and entities crossing boundaries.
- **RCPW-15-R03:** Roads, rivers, trains, projectiles, weather fronts, pursuits, and mission actors remain continuous across dynamic shells.
...
- **RCPW-15-R08:** Unfolding priority accounts for line-of-travel uncertainty and camera look-ahead separately.
- **RCPW-15-R09:** Teleport destination staging validates canonical time, occupancy, collision, and active mission constraints.
- **RCPW-15-R10:** Continuous moving phenomena crossing the player preserve phase/state through fold transitions.
...
- **RCPW-15-R13:** Long-lived projectiles, trains, herds, storms, fires, and other moving phenomena preserve phase and causality through fold/unfold.
...
- **RCPW-15-R16:** Moving transport may become a temporary reference/authority well so passengers, cargo, nearby hazards, and upcoming route segments receive coherent fidelity.
...
- **RCPW-15-R20:** High-speed traversal prewarm is coordinated across partitions/storage tiers so forward world expansion does not produce a thundering-herd resource failure.
...
- **RCPW-15-R26:** Traversal prediction accounts for frontier generation barriers and may slow/redirect presentation rather than invent unvalidated canonical world content.
...
- **RCPW-15-R28:** Moving phenomena encountering an expansion or archive boundary use explicit defer/rehydrate/escalate policy.
- **RCPW-15-R29:** Qualification covers fast travel across old/new world boundaries, archived-route rehydration, infrastructure history, and reference switching during generation/rehydration.

## Stage-Specific Engineering Workflow

1. Implement future reference prediction using velocity, acceleration, route graph, camera direction, vehicle mode, and explicit teleport intent.
2. Allocate forward shell expansion and materialization queues according to time-to-arrival.
3. Collapse trailing geometry/embodiment into lower representations without deleting authoritative state.
4. Implement continuity contracts for moving entities and phenomena that straddle fold boundaries.
5. Implement bounded speculative prefetch/materialization and cancel safely when predictions change.
6. Define emergency degradation order for missed deadlines or resource saturation.
7. Implement teleport staging: freeze authoritative handoff, stage destination, reconcile causal time, establish reference frame, then resume.
8. Benchmark increasing traversal speeds and record materialization misses, hitching, memory peaks, state errors, and recovery.
...
- `TRAVERSAL_FOLD.mssl`
- `high_speed_fold.lctlc`
- `TRAVERSAL_PREDICTOR.md`
- `TELEPORT_PROTOCOL.md`
- `EMERGENCY_DEGRADE_POLICY.json`
- `TRAVERSAL_BENCHMARK.json`
- `RCPW-15_requirements.json`
- `RCPW-15_tests.log`
- `RCPW-15_audit.md`
- `RCPW-15_manifest.sha256`
...
|---|---|---|---|
| RCPW-15-R01 | Forward materialization lead time is predicted from motion and route/camera intent. | NOT_STARTED | — |
| RCPW-15-R02 | Rear folding preserves persistent state and entities crossing boundaries. | NOT_STARTED | — |
| RCPW-15-R03 | Roads, rivers, trains, projectiles, weather fronts, pursuits, and mission actors remain continuous across dynamic shells. | NOT_STARTED | — |
| RCPW-15-R04 | Teleportation has a deterministic destination-staging protocol. | NOT_STARTED | — |
| RCPW-15-R05 | Missed prewarm deadlines degrade presentation safely. | NOT_STARTED | — |
| RCPW-15-R06 | High-speed traversal remains within declared frame, memory, and I/O budgets. | NOT_STARTED | — |
| RCPW-15-R07 | Traversal prediction has bounded speculation windows and backpressure. | NOT_STARTED | — |
| RCPW-15-R08 | Unfolding priority accounts for line-of-travel uncertainty and camera look-ahead separately. | NOT_STARTED | — |
| RCPW-15-R09 | Teleport destination staging validates canonical time, occupancy, collision, and active mission constraints. | NOT_STARTED | — |
| RCPW-15-R10 | Continuous moving phenomena crossing the player preserve phase/state through fold transitions. | NOT_STARTED | — |

| RCPW-15-R11 | High-speed traversal predicts movement and camera look-ahead separately to reduce over-prefetch and preserve directional uncertainty. | NOT_STARTED | — |
| RCPW-15-R12 | Transport networks, tunnels, bridges, portals, and interiors receive route-aware prewarm rather than distance-only prewarm. | NOT_STARTED | — |
| RCPW-15-R13 | Long-lived projectiles, trains, herds, storms, fires, and other moving phenomena preserve phase and causality through fold/unfold. | NOT_STARTED | — |
| RCPW-15-R14 | Emergency degradation exposes a measurable recovery path back to target fidelity after overload ends. | NOT_STARTED | — |

| RCPW-15-R15 | High-speed traversal integrates route-aware prediction from canonical navigation graphs, not only velocity extrapolation. | NOT_STARTED | — |
| RCPW-15-R16 | Moving transport may become a temporary reference/authority well so passengers, cargo, nearby hazards, and upcoming route segments receive coherent fidelity. | NOT_STARTED | — |
| RCPW-15-R17 | Traversal prewarm treats portals, tunnels, bridges, stations, dense settlements, and scripted/event barriers as high-cost milestones. | NOT_STARTED | — |
| RCPW-15-R18 | Backpressure can reduce speculative look-ahead without delaying canonical transport or world time. | NOT_STARTED | — |
| RCPW-15-R19 | Qualification covers mounts, fast ground transport, rail-like transport, vertical transport, portal travel, teleportation, and rapid reference switching. | NOT_STARTED | — |

| RCPW-15-R20 | High-speed traversal prewarm is coordinated across partitions/storage tiers so forward world expansion does not produce a thundering-herd resource failure. | NOT_STARTED | — |
| RCPW-15-R21 | Route milestones carry predicted arrival windows and priority classes for deterministic cross-worker prewarm. | NOT_STARTED | — |
| RCPW-15-R22 | Transport/reference wells can migrate execution ownership ahead of the player while preserving canonical route phase and passenger state. | NOT_STARTED | — |
| RCPW-15-R23 | Speculative prewarm is cancellable and resource-bounded; canonical simulation never waits on cosmetic asset fetch. | NOT_STARTED | — |
| RCPW-15-R24 | Traversal qualification includes cross-partition rail/road/portal travel, worker migration, packet/message delay simulation, and sudden direction reversal. | NOT_STARTED | — |

| RCPW-15-R25 | High-speed traversal into newly generated or long-archived regions triggers semantic validation/rehydration ahead of ordinary visual prewarm. | NOT_STARTED | — |
| RCPW-15-R26 | Traversal prediction accounts for frontier generation barriers and may slow/redirect presentation rather than invent unvalidated canonical world content. | NOT_STARTED | — |
| RCPW-15-R27 | Long-distance transport remains governed by canonical route history, including routes built, destroyed, abandoned, or restored across eras. | NOT_STARTED | — |
| RCPW-15-R28 | Moving phenomena encountering an expansion or archive boundary use explicit defer/rehydrate/escalate policy. | NOT_STARTED | — |
| RCPW-15-R29 | Qualification covers fast travel across old/new world boundaries, archived-route rehydration, infrastructure history, and reference switching during generation/rehydration. | NOT_STARTED | — |
