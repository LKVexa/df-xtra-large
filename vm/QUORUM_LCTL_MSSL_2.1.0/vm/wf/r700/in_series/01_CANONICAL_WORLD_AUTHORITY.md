...
Establish the immutable persistent source of truth for world coordinates, entity identity, terrain, settlements, ecology, inventories, economy, environment, time, and historical state.
...
- **RCPW-01-R01:** Canonical coordinates and units are explicit, versioned, precision-bounded, and independent of render/reference coordinates.
- **RCPW-01-R02:** Every persistent entity has a globally stable identity and versioned schema.
- **RCPW-01-R03:** All authoritative mutations are transactional, attributable, ledger-linked, and replayable.
- **RCPW-01-R04:** Snapshots plus causal ledger ranges reconstruct authoritative state deterministically.
...
- **RCPW-01-R07:** Canonical mutations are totally ordered or transactionally conflict-resolved under concurrent subsystem activity.
...
- **RCPW-01-R12:** Procedural or authored world generation uses deterministic seed/provenance records so regenerated content can be distinguished from hand-authored canonical content.
- **RCPW-01-R13:** Every canonical content object records provenance, schema version, ownership authority, and whether it is authored, generated, migrated, imported, or reconstructed.
...
- **RCPW-01-R15:** Canonical world authority is ingestible from a deterministic world-package manifest that enumerates regions, topology, authored content, generated content, schemas, seeds, dependencies, and hashes.
- **RCPW-01-R16:** Canonical state supports content-pack activation/deactivation transactions without orphaning identities, ownership, routes, or causal references.
...
- **RCPW-01-R19:** World-package validation detects missing dependencies, hash mismatch, duplicate identity, incompatible schema, invalid topology, and forbidden authority cycles before runtime boot.

- **RCPW-01-R20:** Canonical authority is partition-aware but partition-neutral: moving a region/entity between storage or execution partitions cannot change canonical identity, topology, time, ownership, or history.
- **RCPW-01-R21:** Cross-partition canonical mutations use explicit transactions, revision checks, commit/abort semantics, and deterministic conflict resolution.
- **RCPW-01-R22:** World-package activation is atomic at the dependency-set level so partially activated content cannot become canonical truth.
- **RCPW-01-R23:** Canonical state exposes provenance from source content through compiler artifact, runtime partition, mutation event, checkpoint, and replay digest.
...
- **RCPW-01-R25:** Canonical world authority supports append-only world expansion by adding new regions/content namespaces without reassigning existing canonical identities or coordinates.
...
- **RCPW-01-R27:** Canonical region retirement/archive is reversible through provenance-backed rehydration and never deletes unresolved causal obligations.
- **RCPW-01-R28:** Canonical semantic state records ontology/type versions so evolving world concepts remain interpretable across long-lived saves and historical queries.
- **RCPW-01-R29:** World expansion and archival operations produce semantic before/after digests proving unchanged pre-existing canonical regions outside the declared mutation set.

## Stage-Specific Engineering Workflow

1. Inventory every persistent state domain and classify it as canonical, derived, ephemeral, cached, or external.
2. Define canonical spatial reference, units, precision, region addressing, timebase, deterministic seed policy, and world epoch.
3. Define permanent entity IDs, component schemas, parent/child ownership, tombstones, legitimate creation/destruction, and version migration.
4. Implement transactional canonical mutation boundaries with preconditions, postconditions, ledger event emission, and rollback.
5. Implement snapshots, incremental checkpoints, content hashes, schema versions, and replay from a trusted checkpoint.
6. Expose capability-bounded read/write interfaces to reference, fold, LOD, ecology, economy, narrative, materialization, rendering, and save authorities.
7. Add corruption, duplicate-ID, stale-version, partial-write, rollback, and replay-divergence tests.
8. Run long-cycle fold/unfold and save/replay campaigns proving canonical coordinates and identities remain unchanged except by authorized world events.
...
- `CANONICAL_STATE.mssl`
- `canonical_world.lctlc`
- `CANONICAL_SCHEMA.json`
- `MUTATION_CONTRACT.md`
- `SNAPSHOT_REPLAY_SPEC.md`
- `CANONICAL_AUTHORITY_LEDGER.json`
- `RCPW-01_requirements.json`
- `RCPW-01_tests.log`
- `RCPW-01_audit.md`
- `RCPW-01_manifest.sha256`
...
|---|---|---|---|
| RCPW-01-R01 | Canonical coordinates and units are explicit, versioned, precision-bounded, and independent of render/reference coordinates. | NOT_STARTED | — |
| RCPW-01-R02 | Every persistent entity has a globally stable identity and versioned schema. | NOT_STARTED | — |
| RCPW-01-R03 | All authoritative mutations are transactional, attributable, ledger-linked, and replayable. | NOT_STARTED | — |
| RCPW-01-R04 | Snapshots plus causal ledger ranges reconstruct authoritative state deterministically. | NOT_STARTED | — |
| RCPW-01-R05 | Derived rendering, physics, cache, and reference-space systems cannot mutate canonical truth outside declared APIs. | NOT_STARTED | — |
| RCPW-01-R06 | Schema migration and rollback preserve identity and causal history across versions. | NOT_STARTED | — |
| RCPW-01-R07 | Canonical mutations are totally ordered or transactionally conflict-resolved under concurrent subsystem activity. | NOT_STARTED | — |
| RCPW-01-R08 | Canonical state exposes monotonic revision/epoch numbers for stale-read and stale-write detection. | NOT_STARTED | — |
| RCPW-01-R09 | World-state snapshots are quiescent or use a documented consistency algorithm that proves cross-component coherence. | NOT_STARTED | — |
| RCPW-01-R10 | Canonical state recovery is tested across power-loss-equivalent interruption at every write boundary. | NOT_STARTED | — |

| RCPW-01-R11 | Canonical world state includes a versioned spatial-semantic region graph linking regions, biomes, routes, waterways, settlements, interiors, landmarks, and jurisdictional domains. | NOT_STARTED | — |
| RCPW-01-R12 | Procedural or authored world generation uses deterministic seed/provenance records so regenerated content can be distinguished from hand-authored canonical content. | NOT_STARTED | — |
| RCPW-01-R13 | Every canonical content object records provenance, schema version, ownership authority, and whether it is authored, generated, migrated, imported, or reconstructed. | NOT_STARTED | — |
| RCPW-01-R14 | Partial-region availability, damaged content packs, and missing optional content fail without invalidating unrelated canonical regions. | NOT_STARTED | — |

| RCPW-01-R15 | Canonical world authority is ingestible from a deterministic world-package manifest that enumerates regions, topology, authored content, generated content, schemas, seeds, dependencies, and hashes. | NOT_STARTED | — |
| RCPW-01-R16 | Canonical state supports content-pack activation/deactivation transactions without orphaning identities, ownership, routes, or causal references. | NOT_STARTED | — |
| RCPW-01-R17 | Canonical revisions distinguish world-content revision, runtime-state revision, schema revision, and simulation epoch. | NOT_STARTED | — |
| RCPW-01-R18 | Canonical authority provides a content-addressed lookup path for immutable authored assets and a revisioned lookup path for mutable world state. | NOT_STARTED | — |
| RCPW-01-R19 | World-package validation detects missing dependencies, hash mismatch, duplicate identity, incompatible schema, invalid topology, and forbidden authority cycles before runtime boot. | NOT_STARTED | — |

| RCPW-01-R20 | Canonical authority is partition-aware but partition-neutral: moving a region/entity between storage or execution partitions cannot change canonical identity, topology, time, ownership, or history. | NOT_STARTED | — |
| RCPW-01-R21 | Cross-partition canonical mutations use explicit transactions, revision checks, commit/abort semantics, and deterministic conflict resolution. | NOT_STARTED | — |
| RCPW-01-R22 | World-package activation is atomic at the dependency-set level so partially activated content cannot become canonical truth. | NOT_STARTED | — |
| RCPW-01-R23 | Canonical state exposes provenance from source content through compiler artifact, runtime partition, mutation event, checkpoint, and replay digest. | NOT_STARTED | — |
| RCPW-01-R24 | A canonical-state verifier can validate identity uniqueness, topology closure, revision consistency, ledger correspondence, and partition ownership without rendering the world. | NOT_STARTED | — |

| RCPW-01-R25 | Canonical world authority supports append-only world expansion by adding new regions/content namespaces without reassigning existing canonical identities or coordinates. | NOT_STARTED | — |
| RCPW-01-R26 | Canonical world history distinguishes world age, era, content generation epoch, and simulation epoch so later synthesis cannot silently rewrite earlier truth. | NOT_STARTED | — |
| RCPW-01-R27 | Canonical region retirement/archive is reversible through provenance-backed rehydration and never deletes unresolved causal obligations. | NOT_STARTED | — |
| RCPW-01-R28 | Canonical semantic state records ontology/type versions so evolving world concepts remain interpretable across long-lived saves and historical queries. | NOT_STARTED | — |
| RCPW-01-R29 | World expansion and archival operations produce semantic before/after digests proving unchanged pre-existing canonical regions outside the declared mutation set. | NOT_STARTED | — |
