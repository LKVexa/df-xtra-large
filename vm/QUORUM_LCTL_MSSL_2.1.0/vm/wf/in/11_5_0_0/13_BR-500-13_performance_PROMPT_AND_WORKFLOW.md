...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Performance** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-13-R01:** Publish benchmark baseline
- **BR-500-13-R02:** Bound startup
- **BR-500-13-R03:** Bound verification
- **BR-500-13-R04:** Bound memory
- **BR-500-13-R05:** Optimize wide arithmetic
...
Define the invariants for **Performance** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-13-R01 — Publish benchmark baseline.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-500-13-R02 — Bound startup.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-13-R03 — Bound verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-13-R04 — Bound memory.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-500-13-R05 — Optimize wide arithmetic.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-13_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-13_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-13_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-13_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-13-R01 | Publish benchmark baseline | NOT RUN | — |
| BR-500-13-R02 | Bound startup | NOT RUN | — |
| BR-500-13-R03 | Bound verification | NOT RUN | — |
| BR-500-13-R04 | Bound memory | NOT RUN | — |
| BR-500-13-R05 | Optimize wide arithmetic | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-13**. The next package may consume this package only when its declared prerequisite status is satisfied.
