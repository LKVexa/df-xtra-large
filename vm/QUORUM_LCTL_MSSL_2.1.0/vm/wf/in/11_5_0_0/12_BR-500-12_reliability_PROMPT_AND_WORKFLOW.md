...
### Atomic requirements
- **BR-500-12-R01:** Crash recovery passes
- **BR-500-12-R02:** Interrupted-update recovery passes
- **BR-500-12-R03:** Malformed input is contained
- **BR-500-12-R04:** Execution resources are enforced
- **BR-500-12-R05:** 72-hour soak passes
...
Define the invariants for **Reliability** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-12-R01 — Crash recovery passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-500-12-R02 — Interrupted-update recovery passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
3. **BR-500-12-R03 — Malformed input is contained.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-12-R04 — Execution resources are enforced.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-500-12-R05 — 72-hour soak passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-500-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-12-R01 | Crash recovery passes | NOT RUN | — |
| BR-500-12-R02 | Interrupted-update recovery passes | NOT RUN | — |
| BR-500-12-R03 | Malformed input is contained | NOT RUN | — |
| BR-500-12-R04 | Execution resources are enforced | NOT RUN | — |
| BR-500-12-R05 | 72-hour soak passes | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
