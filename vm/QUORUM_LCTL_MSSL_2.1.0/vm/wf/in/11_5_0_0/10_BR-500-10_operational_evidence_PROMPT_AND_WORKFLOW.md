# QUORUM BR-500-10 — Operational evidence
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Operational evidence** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-10-R01:** Generate full automated ledger
- **BR-500-10-R02:** All mandatory tests PASS
- **BR-500-10-R03:** Hash all evidence
- **BR-500-10-R04:** Record exact build provenance
- **BR-500-10-R05:** Independent replay PASS
...
Define the invariants for **Operational evidence** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-10-R01 — Generate full automated ledger.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-10-R02 — All mandatory tests PASS.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-500-10-R03 — Hash all evidence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-10-R04 — Record exact build provenance.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-500-10-R05 — Independent replay PASS.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-500-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-10-R01 | Generate full automated ledger | NOT RUN | — |
| BR-500-10-R02 | All mandatory tests PASS | NOT RUN | — |
| BR-500-10-R03 | Hash all evidence | NOT RUN | — |
| BR-500-10-R04 | Record exact build provenance | NOT RUN | — |
| BR-500-10-R05 | Independent replay PASS | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
