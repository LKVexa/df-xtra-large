# QUORUM BR-500-06 — Production persistence
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production persistence** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-06-R01:** Transactional persistence
- **BR-500-06-R02:** Authenticated persistence
- **BR-500-06-R03:** Crash-safe persistence
- **BR-500-06-R04:** Recoverable persistence
- **BR-500-06-R05:** Monotonic persistence
...
Define the invariants for **Production persistence** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-06-R01 — Transactional persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-06-R02 — Authenticated persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-06-R03 — Crash-safe persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-06-R04 — Recoverable persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-500-06-R05 — Monotonic persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-06-R01 | Transactional persistence | NOT RUN | — |
| BR-500-06-R02 | Authenticated persistence | NOT RUN | — |
| BR-500-06-R03 | Crash-safe persistence | NOT RUN | — |
| BR-500-06-R04 | Recoverable persistence | NOT RUN | — |
| BR-500-06-R05 | Monotonic persistence | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
