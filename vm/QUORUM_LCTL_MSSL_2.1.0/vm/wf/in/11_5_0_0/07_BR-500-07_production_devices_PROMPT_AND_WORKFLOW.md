# QUORUM BR-500-07 — Production devices
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production devices** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-07-R01:** Versioned device ABI
- **BR-500-07-R02:** Capability gating
- **BR-500-07-R03:** Bounded I/O
- **BR-500-07-R04:** Deterministic replay support
...
Define the invariants for **Production devices** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-07-R01 — Versioned device ABI.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-07-R02 — Capability gating.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-07-R03 — Bounded I/O.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-07-R04 — Deterministic replay support.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-500-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-07-R01 | Versioned device ABI | NOT RUN | — |
| BR-500-07-R02 | Capability gating | NOT RUN | — |
| BR-500-07-R03 | Bounded I/O | NOT RUN | — |
| BR-500-07-R04 | Deterministic replay support | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
