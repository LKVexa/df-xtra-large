# QUORUM BR-500-04 — Production VM core
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production VM core** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-04-R01:** Use stable ISA
- **BR-500-04-R02:** Use stable ABI
- **BR-500-04-R03:** Use isolated VM state
- **BR-500-04-R04:** Provide complete traps
- **BR-500-04-R05:** Bound resources
- **BR-500-04-R06:** Guarantee deterministic semantics
...
Define the invariants for **Production VM core** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-04-R01 — Use stable ISA.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-500-04-R02 — Use stable ABI.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-500-04-R03 — Use isolated VM state.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-500-04-R04 — Provide complete traps.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-500-04-R05 — Bound resources.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-500-04-R06 — Guarantee deterministic semantics.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-04-R01 | Use stable ISA | NOT RUN | — |
| BR-500-04-R02 | Use stable ABI | NOT RUN | — |
| BR-500-04-R03 | Use isolated VM state | NOT RUN | — |
| BR-500-04-R04 | Provide complete traps | NOT RUN | — |
| BR-500-04-R05 | Bound resources | NOT RUN | — |
| BR-500-04-R06 | Guarantee deterministic semantics | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
