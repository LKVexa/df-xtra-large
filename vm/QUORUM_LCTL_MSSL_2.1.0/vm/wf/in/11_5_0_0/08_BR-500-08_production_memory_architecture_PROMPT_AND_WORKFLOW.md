# QUORUM BR-500-08 — Production memory architecture
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production memory architecture** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-08-R01:** Use lazy wide words
- **BR-500-08-R02:** Use bounded stack
- **BR-500-08-R03:** Use bounded scratch
- **BR-500-08-R04:** Provide predictable resource consumption
- **BR-500-08-R05:** Retain megabit values
...
Define the invariants for **Production memory architecture** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-08-R01 — Use lazy wide words.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-500-08-R02 — Use bounded stack.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-500-08-R03 — Use bounded scratch.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-500-08-R04 — Provide predictable resource consumption.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-500-08-R05 — Retain megabit values.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-08-R01 | Use lazy wide words | NOT RUN | — |
| BR-500-08-R02 | Use bounded stack | NOT RUN | — |
| BR-500-08-R03 | Use bounded scratch | NOT RUN | — |
| BR-500-08-R04 | Provide predictable resource consumption | NOT RUN | — |
| BR-500-08-R05 | Retain megabit values | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
