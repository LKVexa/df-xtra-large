...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Security** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-09-R01:** Use immutable trust root
- **BR-500-09-R02:** Support key rotation
- **BR-500-09-R03:** Support revocation
- **BR-500-09-R04:** Use secure boot
- **BR-500-09-R05:** Use signed updates
- **BR-500-09-R06:** Prevent rollback
- **BR-500-09-R07:** Enforce isolation
...
Define the invariants for **Security** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-09-R01 — Use immutable trust root.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-500-09-R02 — Support key rotation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-500-09-R03 — Support revocation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-500-09-R04 — Use secure boot.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-500-09-R05 — Use signed updates.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-500-09-R06 — Prevent rollback.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
7. **BR-500-09-R07 — Enforce isolation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-09-R01 | Use immutable trust root | NOT RUN | — |
| BR-500-09-R02 | Support key rotation | NOT RUN | — |
| BR-500-09-R03 | Support revocation | NOT RUN | — |
| BR-500-09-R04 | Use secure boot | NOT RUN | — |
| BR-500-09-R05 | Use signed updates | NOT RUN | — |
| BR-500-09-R06 | Prevent rollback | NOT RUN | — |
| BR-500-09-R07 | Enforce isolation | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
