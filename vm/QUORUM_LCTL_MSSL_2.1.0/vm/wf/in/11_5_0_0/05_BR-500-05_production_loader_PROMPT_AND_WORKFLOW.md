# QUORUM BR-500-05 — Production loader
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production loader** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-05-R01:** Allow signed images only
- **BR-500-05-R02:** Enforce rollback resistance
- **BR-500-05-R03:** Derive capabilities
- **BR-500-05-R04:** Enforce strict version compatibility
...
Define the invariants for **Production loader** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-05-R01 — Allow signed images only.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-05-R02 — Enforce rollback resistance.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-05-R03 — Derive capabilities.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-500-05-R04 — Enforce strict version compatibility.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-05-R01 | Allow signed images only | NOT RUN | — |
| BR-500-05-R02 | Enforce rollback resistance | NOT RUN | — |
| BR-500-05-R03 | Derive capabilities | NOT RUN | — |
| BR-500-05-R04 | Enforce strict version compatibility | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
