# QUORUM BR-500-01 — Native source authority
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Native source authority** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-01-R01:** COLUMNED LCTL is authoritative
- **BR-500-01-R02:** Language specification frozen
- **BR-500-01-R03:** Canonical verifier operational
...
Define the invariants for **Native source authority** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-01-R01 — COLUMNED LCTL is authoritative.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-01-R02 — Language specification frozen.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-01-R03 — Canonical verifier operational.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-500-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-01-R01 | COLUMNED LCTL is authoritative | NOT RUN | — |
| BR-500-01-R02 | Language specification frozen | NOT RUN | — |
| BR-500-01-R03 | Canonical verifier operational | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
