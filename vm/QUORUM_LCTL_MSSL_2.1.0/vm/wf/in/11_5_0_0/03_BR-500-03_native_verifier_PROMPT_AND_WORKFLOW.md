# QUORUM BR-500-03 — Native verifier
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Native verifier** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-500-03-R01:** Perform structural verification
- **BR-500-03-R02:** Perform semantic verification
- **BR-500-03-R03:** Perform capability verification
- **BR-500-03-R04:** Perform control-flow verification
- **BR-500-03-R05:** Perform image verification
...
Define the invariants for **Native verifier** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-500-03-R01 — Perform structural verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-500-03-R02 — Perform semantic verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-03-R03 — Perform capability verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-500-03-R04 — Perform control-flow verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-500-03-R05 — Perform image verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-03-R01 | Perform structural verification | NOT RUN | — |
| BR-500-03-R02 | Perform semantic verification | NOT RUN | — |
| BR-500-03-R03 | Perform capability verification | NOT RUN | — |
| BR-500-03-R04 | Perform control-flow verification | NOT RUN | — |
| BR-500-03-R05 | Perform image verification | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
