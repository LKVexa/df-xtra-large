# QUORUM BR-500-11 — Reproducibility
...
### Atomic requirements
- **BR-500-11-R01:** Independent rebuild passes
- **BR-500-11-R02:** BRIM is deterministic
- **BR-500-11-R03:** Test outputs are deterministic
- **BR-500-11-R04:** Build provenance complete
...
### Q2 — Implementation
1. **BR-500-11-R01 — Independent rebuild passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-500-11-R02 — BRIM is deterministic.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-500-11-R03 — Test outputs are deterministic.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-500-11-R04 — Build provenance complete.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-500-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-500-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-500-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-500-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-500-11-R01 | Independent rebuild passes | NOT RUN | — |
| BR-500-11-R02 | BRIM is deterministic | NOT RUN | — |
| BR-500-11-R03 | Test outputs are deterministic | NOT RUN | — |
| BR-500-11-R04 | Build provenance complete | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-500-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
