# QUORUM BR-490-04 — Core-61 qualification
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Core-61 qualification** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-490-04-R01:** Define exactly what is counted
- **BR-490-04-R02:** Strip production core
- **BR-490-04-R03:** Measure byte-for-byte
- **BR-490-04-R04:** Keep under agreed 61,000-byte core ceiling if achievable
- **BR-490-04-R05:** Document exact blocker instead of manipulating packaging if impossible
...
Define the invariants for **Core-61 qualification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-04-R01 — Define exactly what is counted.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-490-04-R02 — Strip production core.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-490-04-R03 — Measure byte-for-byte.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-490-04-R04 — Keep under agreed 61,000-byte core ceiling if achievable.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-490-04-R05 — Document exact blocker instead of manipulating packaging if impossible.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-490-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-04-R01 | Define exactly what is counted | NOT RUN | — |
| BR-490-04-R02 | Strip production core | NOT RUN | — |
| BR-490-04-R03 | Measure byte-for-byte | NOT RUN | — |
| BR-490-04-R04 | Keep under agreed 61,000-byte core ceiling if achievable | NOT RUN | — |
| BR-490-04-R05 | Document exact blocker instead of manipulating packaging if impossible | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
