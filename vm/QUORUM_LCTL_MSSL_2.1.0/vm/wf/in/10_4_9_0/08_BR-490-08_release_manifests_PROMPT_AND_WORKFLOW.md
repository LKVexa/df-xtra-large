# QUORUM BR-490-08 — Release manifests
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Release manifests** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-490-08-R01:** Record source hash
- **BR-490-08-R02:** Record binary hash
- **BR-490-08-R03:** Record BRIM hash
- **BR-490-08-R04:** Record compiler hash
- **BR-490-08-R05:** Record verifier hash
- **BR-490-08-R06:** Record test-suite hash
- **BR-490-08-R07:** Record specification hash
- **BR-490-08-R08:** Record evidence hash
...
Define the invariants for **Release manifests** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-08-R01 — Record source hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
2. **BR-490-08-R02 — Record binary hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-490-08-R03 — Record BRIM hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-490-08-R04 — Record compiler hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-490-08-R05 — Record verifier hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-490-08-R06 — Record test-suite hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
7. **BR-490-08-R07 — Record specification hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-490-08-R08 — Record evidence hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-490-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-08-R01 | Record source hash | NOT RUN | — |
| BR-490-08-R02 | Record binary hash | NOT RUN | — |
| BR-490-08-R03 | Record BRIM hash | NOT RUN | — |
| BR-490-08-R04 | Record compiler hash | NOT RUN | — |
| BR-490-08-R05 | Record verifier hash | NOT RUN | — |
| BR-490-08-R06 | Record test-suite hash | NOT RUN | — |
| BR-490-08-R07 | Record specification hash | NOT RUN | — |
| BR-490-08-R08 | Record evidence hash | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
