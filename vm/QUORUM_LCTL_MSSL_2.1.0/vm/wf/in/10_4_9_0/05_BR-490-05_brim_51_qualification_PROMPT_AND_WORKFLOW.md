# QUORUM BR-490-05 — BRIM-51 qualification
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **BRIM-51 qualification** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.h::BR_UPDATE_BYTES`
- `Makefile::verify`

### Atomic requirements
- **BR-490-05-R01:** Define 51,200-byte executable image profile if retained
- **BR-490-05-R02:** Enforce limit in compiler
- **BR-490-05-R03:** Enforce limit in loader
- **BR-490-05-R04:** Test oversized-image rejection
...
Define the invariants for **BRIM-51 qualification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-05-R01 — Define 51,200-byte executable image profile if retained.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-490-05-R02 — Enforce limit in compiler.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-490-05-R03 — Enforce limit in loader.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-490-05-R04 — Test oversized-image rejection.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-490-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-05-R01 | Define 51,200-byte executable image profile if retained | NOT RUN | — |
| BR-490-05-R02 | Enforce limit in compiler | NOT RUN | — |
| BR-490-05-R03 | Enforce limit in loader | NOT RUN | — |
| BR-490-05-R04 | Test oversized-image rejection | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
