# QUORUM BR-490-06 — Runtime-memory qualification
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Runtime-memory qualification** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `host/br_sizes.c`
- `src/brvm.c::br_vm_run/service_call`
- `src/brvm.c::trap`

### Atomic requirements
- **BR-490-06-R01:** Measure idle RAM
- **BR-490-06-R02:** Measure typical RAM
- **BR-490-06-R03:** Measure maximum configured RAM
- **BR-490-06-R04:** Measure worst-case maximum-width RAM
- **BR-490-06-R05:** Measure peak RSS
- **BR-490-06-R06:** Verify memory-quota behavior
...
Define the invariants for **Runtime-memory qualification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-06-R01 — Measure idle RAM.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
2. **BR-490-06-R02 — Measure typical RAM.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-490-06-R03 — Measure maximum configured RAM.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-490-06-R04 — Measure worst-case maximum-width RAM.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-490-06-R05 — Measure peak RSS.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-490-06-R06 — Verify memory-quota behavior.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-490-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-06-R01 | Measure idle RAM | NOT RUN | — |
| BR-490-06-R02 | Measure typical RAM | NOT RUN | — |
| BR-490-06-R03 | Measure maximum configured RAM | NOT RUN | — |
| BR-490-06-R04 | Measure worst-case maximum-width RAM | NOT RUN | — |
| BR-490-06-R05 | Measure peak RSS | NOT RUN | — |
| BR-490-06-R06 | Verify memory-quota behavior | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
