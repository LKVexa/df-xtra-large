# QUORUM BR-490-01 — Feature freeze
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Feature freeze** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-490-01-R01:** Freeze ISA
- **BR-490-01-R02:** Freeze ABI
- **BR-490-01-R03:** Freeze BRIM format
- **BR-490-01-R04:** Freeze COLUMNED LCTL profile
- **BR-490-01-R05:** Freeze host ABI
- **BR-490-01-R06:** Freeze device ABI
...
Define the invariants for **Feature freeze** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-01-R01 — Freeze ISA.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-490-01-R02 — Freeze ABI.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
3. **BR-490-01-R03 — Freeze BRIM format.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
4. **BR-490-01-R04 — Freeze COLUMNED LCTL profile.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
5. **BR-490-01-R05 — Freeze host ABI.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
6. **BR-490-01-R06 — Freeze device ABI.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-490-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-01-R01 | Freeze ISA | NOT RUN | — |
| BR-490-01-R02 | Freeze ABI | NOT RUN | — |
| BR-490-01-R03 | Freeze BRIM format | NOT RUN | — |
| BR-490-01-R04 | Freeze COLUMNED LCTL profile | NOT RUN | — |
| BR-490-01-R05 | Freeze host ABI | NOT RUN | — |
| BR-490-01-R06 | Freeze device ABI | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
