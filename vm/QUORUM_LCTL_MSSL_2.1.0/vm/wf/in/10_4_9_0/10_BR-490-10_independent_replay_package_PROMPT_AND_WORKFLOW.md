# QUORUM BR-490-10 — Independent replay package
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Independent replay package** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-490-10-R01:** Include source
- **BR-490-10-R02:** Include build recipe
- **BR-490-10-R03:** Include deterministic inputs
- **BR-490-10-R04:** Include expected hashes
- **BR-490-10-R05:** Include tests
- **BR-490-10-R06:** Include expected outputs
- **BR-490-10-R07:** Include evidence verifier
...
Define the invariants for **Independent replay package** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-10-R01 — Include source.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-490-10-R02 — Include build recipe.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-490-10-R03 — Include deterministic inputs.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-490-10-R04 — Include expected hashes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-490-10-R05 — Include tests.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
6. **BR-490-10-R06 — Include expected outputs.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-490-10-R07 — Include evidence verifier.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-490-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-10-R01 | Include source | NOT RUN | — |
| BR-490-10-R02 | Include build recipe | NOT RUN | — |
| BR-490-10-R03 | Include deterministic inputs | NOT RUN | — |
| BR-490-10-R04 | Include expected hashes | NOT RUN | — |
| BR-490-10-R05 | Include tests | NOT RUN | — |
| BR-490-10-R06 | Include expected outputs | NOT RUN | — |
| BR-490-10-R07 | Include evidence verifier | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
