# QUORUM BR-490-11 — Final RC acceptance
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Final RC acceptance** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::br_selftest`

### Atomic requirements
- **BR-490-11-R01:** Clean-room build passes
- **BR-490-11-R02:** Full conformance suite passes
- **BR-490-11-R03:** Security suite passes
- **BR-490-11-R04:** Fuzz regression suite passes
- **BR-490-11-R05:** Recovery suite passes
- **BR-490-11-R06:** Performance suite passes
- **BR-490-11-R07:** Resource suite passes
- **BR-490-11-R08:** 72-hour soak passes
- **BR-490-11-R09:** Independent replay passes
...
Define the invariants for **Final RC acceptance** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-11-R01 — Clean-room build passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-490-11-R02 — Full conformance suite passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
3. **BR-490-11-R03 — Security suite passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
4. **BR-490-11-R04 — Fuzz regression suite passes.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-490-11-R05 — Recovery suite passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
6. **BR-490-11-R06 — Performance suite passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
7. **BR-490-11-R07 — Resource suite passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
8. **BR-490-11-R08 — 72-hour soak passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
9. **BR-490-11-R09 — Independent replay passes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-490-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-11-R01 | Clean-room build passes | NOT RUN | — |
| BR-490-11-R02 | Full conformance suite passes | NOT RUN | — |
| BR-490-11-R03 | Security suite passes | NOT RUN | — |
| BR-490-11-R04 | Fuzz regression suite passes | NOT RUN | — |
| BR-490-11-R05 | Recovery suite passes | NOT RUN | — |
| BR-490-11-R06 | Performance suite passes | NOT RUN | — |
| BR-490-11-R07 | Resource suite passes | NOT RUN | — |
| BR-490-11-R08 | 72-hour soak passes | NOT RUN | — |
| BR-490-11-R09 | Independent replay passes | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
