# QUORUM BR-490-03 — Production hardening
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production hardening** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `host/brctl.c::runplain/verifyrun/update`
- `host/brctl.c::keygen/signimg`
- `src/brvm.c::br_vm_set_issuer_key`
- `src/brvm.c::diag_add`
- `src/brvm.h::br_diag`

### Atomic requirements
- **BR-490-03-R01:** Remove debug bypasses
- **BR-490-03-R02:** Remove unsafe loaders
- **BR-490-03-R03:** Remove test signing keys
- **BR-490-03-R04:** Remove assertions that leak secrets
- **BR-490-03-R05:** Strip symbols where required
- **BR-490-03-R06:** Restrict diagnostics
- **BR-490-03-R07:** Set secure defaults
...
Define the invariants for **Production hardening** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-03-R01 — Remove debug bypasses.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
2. **BR-490-03-R02 — Remove unsafe loaders.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-490-03-R03 — Remove test signing keys.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
4. **BR-490-03-R04 — Remove assertions that leak secrets.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
5. **BR-490-03-R05 — Strip symbols where required.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
6. **BR-490-03-R06 — Restrict diagnostics.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-490-03-R07 — Set secure defaults.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-490-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-03-R01 | Remove debug bypasses | NOT RUN | — |
| BR-490-03-R02 | Remove unsafe loaders | NOT RUN | — |
| BR-490-03-R03 | Remove test signing keys | NOT RUN | — |
| BR-490-03-R04 | Remove assertions that leak secrets | NOT RUN | — |
| BR-490-03-R05 | Strip symbols where required | NOT RUN | — |
| BR-490-03-R06 | Restrict diagnostics | NOT RUN | — |
| BR-490-03-R07 | Set secure defaults | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
