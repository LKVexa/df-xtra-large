# QUORUM BR-490-02 — Release profiles
...
### Atomic requirements
- **BR-490-02-R01:** Define development profile
- **BR-490-02-R02:** Define testing profile
- **BR-490-02-R03:** Define production profile
- **BR-490-02-R04:** Define constrained profile
- **BR-490-02-R05:** Define diagnostic profile
- **BR-490-02-R06:** Define recovery profile
...
Define the invariants for **Release profiles** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-02-R01 — Define development profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-490-02-R02 — Define testing profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-490-02-R03 — Define production profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-490-02-R04 — Define constrained profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-490-02-R05 — Define diagnostic profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-490-02-R06 — Define recovery profile.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-490-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-02-R01 | Define development profile | NOT RUN | — |
| BR-490-02-R02 | Define testing profile | NOT RUN | — |
| BR-490-02-R03 | Define production profile | NOT RUN | — |
| BR-490-02-R04 | Define constrained profile | NOT RUN | — |
| BR-490-02-R05 | Define diagnostic profile | NOT RUN | — |
| BR-490-02-R06 | Define recovery profile | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
