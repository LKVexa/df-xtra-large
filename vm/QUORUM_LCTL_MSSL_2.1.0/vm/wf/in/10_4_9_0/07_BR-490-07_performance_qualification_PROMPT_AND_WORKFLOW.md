# QUORUM BR-490-07 — Performance qualification
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Performance qualification** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-490-07-R01:** Qualify startup
- **BR-490-07-R02:** Qualify image verification
- **BR-490-07-R03:** Qualify load
- **BR-490-07-R04:** Qualify execution throughput
- **BR-490-07-R05:** Qualify arithmetic
- **BR-490-07-R06:** Qualify persistence
- **BR-490-07-R07:** Qualify recovery
- **BR-490-07-R08:** Qualify host-call latency
...
Define the invariants for **Performance qualification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-490-07-R01 — Qualify startup.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-490-07-R02 — Qualify image verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-490-07-R03 — Qualify load.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-490-07-R04 — Qualify execution throughput.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-490-07-R05 — Qualify arithmetic.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-490-07-R06 — Qualify persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-490-07-R07 — Qualify recovery.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-490-07-R08 — Qualify host-call latency.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-490-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-490-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-490-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-490-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-490-07-R01 | Qualify startup | NOT RUN | — |
| BR-490-07-R02 | Qualify image verification | NOT RUN | — |
| BR-490-07-R03 | Qualify load | NOT RUN | — |
| BR-490-07-R04 | Qualify execution throughput | NOT RUN | — |
| BR-490-07-R05 | Qualify arithmetic | NOT RUN | — |
| BR-490-07-R06 | Qualify persistence | NOT RUN | — |
| BR-490-07-R07 | Qualify recovery | NOT RUN | — |
| BR-490-07-R08 | Qualify host-call latency | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-490-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
