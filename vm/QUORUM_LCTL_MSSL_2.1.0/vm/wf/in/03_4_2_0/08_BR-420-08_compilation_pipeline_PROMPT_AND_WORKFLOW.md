# QUORUM BR-420-08 — Compilation pipeline
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Compilation pipeline** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-08-R01:** Parse COLUMNED LCTL
- **BR-420-08-R02:** Perform lexical verification
- **BR-420-08-R03:** Perform structural verification
- **BR-420-08-R04:** Perform semantic verification
- **BR-420-08-R05:** Perform capability verification
- **BR-420-08-R06:** Construct BRIR
- **BR-420-08-R07:** Optimize BRIR safely
- **BR-420-08-R08:** Lower BRIR to BRIM
- **BR-420-08-R09:** Serialize deterministically
- **BR-420-08-R10:** Hash artifact
- **BR-420-08-R11:** Sign artifact
...
Define the invariants for **Compilation pipeline** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-08-R01 — Parse COLUMNED LCTL.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-08-R02 — Perform lexical verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-420-08-R03 — Perform structural verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-08-R04 — Perform semantic verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-08-R05 — Perform capability verification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-420-08-R06 — Construct BRIR.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
7. **BR-420-08-R07 — Optimize BRIR safely.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-420-08-R08 — Lower BRIR to BRIM.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
9. **BR-420-08-R09 — Serialize deterministically.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
10. **BR-420-08-R10 — Hash artifact.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
11. **BR-420-08-R11 — Sign artifact.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-08-R01 | Parse COLUMNED LCTL | NOT RUN | — |
| BR-420-08-R02 | Perform lexical verification | NOT RUN | — |
| BR-420-08-R03 | Perform structural verification | NOT RUN | — |
| BR-420-08-R04 | Perform semantic verification | NOT RUN | — |
| BR-420-08-R05 | Perform capability verification | NOT RUN | — |
| BR-420-08-R06 | Construct BRIR | NOT RUN | — |
| BR-420-08-R07 | Optimize BRIR safely | NOT RUN | — |
| BR-420-08-R08 | Lower BRIR to BRIM | NOT RUN | — |
| BR-420-08-R09 | Serialize deterministically | NOT RUN | — |
| BR-420-08-R10 | Hash artifact | NOT RUN | — |
| BR-420-08-R11 | Sign artifact | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
