# QUORUM BR-420-09 — Source-to-binary provenance
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Source-to-binary provenance** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-09-R01:** Embed source hash in manifest
- **BR-420-09-R02:** Record BRIR hash
- **BR-420-09-R03:** Record BRIM hash
- **BR-420-09-R04:** Record verifier version
- **BR-420-09-R05:** Record compiler version
- **BR-420-09-R06:** Record language version
- **BR-420-09-R07:** Record ISA version
...
Define the invariants for **Source-to-binary provenance** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-09-R01 — Embed source hash in manifest.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-09-R02 — Record BRIR hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-420-09-R03 — Record BRIM hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-420-09-R04 — Record verifier version.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-420-09-R05 — Record compiler version.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-420-09-R06 — Record language version.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
7. **BR-420-09-R07 — Record ISA version.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-420-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-09-R01 | Embed source hash in manifest | NOT RUN | — |
| BR-420-09-R02 | Record BRIR hash | NOT RUN | — |
| BR-420-09-R03 | Record BRIM hash | NOT RUN | — |
| BR-420-09-R04 | Record verifier version | NOT RUN | — |
| BR-420-09-R05 | Record compiler version | NOT RUN | — |
| BR-420-09-R06 | Record language version | NOT RUN | — |
| BR-420-09-R07 | Record ISA version | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
