# QUORUM BR-420-04 — Static verifier

**Release sequence:** BOTTLE ROCKET 4.2.0 — Native COLUMNED LCTL Semantic Authority  
**Purpose:** Make COLUMNED LCTL the authoritative executable source rather than a metadata carrier.  
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Static verifier** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-04-R01:** Verify opcode validity
- **BR-420-04-R02:** Verify operand count
- **BR-420-04-R03:** Verify operand type
- **BR-420-04-R04:** Verify register bounds
- **BR-420-04-R05:** Verify memory bounds
- **BR-420-04-R06:** Verify immediate bounds
- **BR-420-04-R07:** Verify branch-target validity
- **BR-420-04-R08:** Verify capability validity
- **BR-420-04-R09:** Verify stack-effect validity
- **BR-420-04-R10:** Verify mode validity
- **BR-420-04-R11:** Detect unreachable code where appropriate
...
Define the invariants for **Static verifier** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-04-R01 — Verify opcode validity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-04-R02 — Verify operand count.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-420-04-R03 — Verify operand type.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-04-R04 — Verify register bounds.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-04-R05 — Verify memory bounds.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-420-04-R06 — Verify immediate bounds.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-420-04-R07 — Verify branch-target validity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-420-04-R08 — Verify capability validity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
9. **BR-420-04-R09 — Verify stack-effect validity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
10. **BR-420-04-R10 — Verify mode validity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
11. **BR-420-04-R11 — Detect unreachable code where appropriate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-04-R01 | Verify opcode validity | NOT RUN | — |
| BR-420-04-R02 | Verify operand count | NOT RUN | — |
| BR-420-04-R03 | Verify operand type | NOT RUN | — |
| BR-420-04-R04 | Verify register bounds | NOT RUN | — |
| BR-420-04-R05 | Verify memory bounds | NOT RUN | — |
| BR-420-04-R06 | Verify immediate bounds | NOT RUN | — |
| BR-420-04-R07 | Verify branch-target validity | NOT RUN | — |
| BR-420-04-R08 | Verify capability validity | NOT RUN | — |
| BR-420-04-R09 | Verify stack-effect validity | NOT RUN | — |
| BR-420-04-R10 | Verify mode validity | NOT RUN | — |
| BR-420-04-R11 | Detect unreachable code where appropriate | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
