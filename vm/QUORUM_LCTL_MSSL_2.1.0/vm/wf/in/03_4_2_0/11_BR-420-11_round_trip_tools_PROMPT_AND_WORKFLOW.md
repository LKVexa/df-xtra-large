# QUORUM BR-420-11 — Round-trip tools
...
- `host/brctl.c::assemble/inspect`

### Atomic requirements
- **BR-420-11-R01:** Implement LCTL to BRIR
- **BR-420-11-R02:** Implement BRIR diagnostic representation
- **BR-420-11-R03:** Implement BRIM disassembly
- **BR-420-11-R04:** Compare disassembly against source semantics
- **BR-420-11-R05:** Verify binary provenance
...
Define the invariants for **Round-trip tools** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-11-R01 — Implement LCTL to BRIR.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-420-11-R02 — Implement BRIR diagnostic representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-420-11-R03 — Implement BRIM disassembly.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-420-11-R04 — Compare disassembly against source semantics.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-11-R05 — Verify binary provenance.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-11-R01 | Implement LCTL to BRIR | NOT RUN | — |
| BR-420-11-R02 | Implement BRIR diagnostic representation | NOT RUN | — |
| BR-420-11-R03 | Implement BRIM disassembly | NOT RUN | — |
| BR-420-11-R04 | Compare disassembly against source semantics | NOT RUN | — |
| BR-420-11-R05 | Verify binary provenance | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
