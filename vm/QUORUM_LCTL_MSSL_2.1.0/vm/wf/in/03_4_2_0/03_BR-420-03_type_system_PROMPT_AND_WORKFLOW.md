...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Type system** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-03-R01:** Define wide word
- **BR-420-03-R02:** Define scalar
- **BR-420-03-R03:** Define address
- **BR-420-03-R04:** Define register
- **BR-420-03-R05:** Define capability
- **BR-420-03-R06:** Define instruction index
- **BR-420-03-R07:** Define service ID
- **BR-420-03-R08:** Define image constant
- **BR-420-03-R09:** Define immutable object
- **BR-420-03-R10:** Define mutable state object
...
Define the invariants for **Type system** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-03-R01 — Define wide word.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-420-03-R02 — Define scalar.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-420-03-R03 — Define address.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-420-03-R04 — Define register.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-420-03-R05 — Define capability.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-420-03-R06 — Define instruction index.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-420-03-R07 — Define service ID.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-420-03-R08 — Define image constant.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
9. **BR-420-03-R09 — Define immutable object.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
10. **BR-420-03-R10 — Define mutable state object.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-420-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-03-R01 | Define wide word | NOT RUN | — |
| BR-420-03-R02 | Define scalar | NOT RUN | — |
| BR-420-03-R03 | Define address | NOT RUN | — |
| BR-420-03-R04 | Define register | NOT RUN | — |
| BR-420-03-R05 | Define capability | NOT RUN | — |
| BR-420-03-R06 | Define instruction index | NOT RUN | — |
| BR-420-03-R07 | Define service ID | NOT RUN | — |
| BR-420-03-R08 | Define image constant | NOT RUN | — |
| BR-420-03-R09 | Define immutable object | NOT RUN | — |
| BR-420-03-R10 | Define mutable state object | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
