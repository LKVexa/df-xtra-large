# QUORUM BR-420-06 — Capability verification
...
### Atomic requirements
- **BR-420-06-R01:** Declare required capabilities
- **BR-420-06-R02:** Derive capability set
- **BR-420-06-R03:** Ensure instructions cannot exceed declared capabilities
- **BR-420-06-R04:** Reject undeclared device access
- **BR-420-06-R05:** Reject undeclared persistent-state access
- **BR-420-06-R06:** Reject undeclared privileged instructions
...
Define the invariants for **Capability verification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-06-R01 — Declare required capabilities.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-420-06-R02 — Derive capability set.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-420-06-R03 — Ensure instructions cannot exceed declared capabilities.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-06-R04 — Reject undeclared device access.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
5. **BR-420-06-R05 — Reject undeclared persistent-state access.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
6. **BR-420-06-R06 — Reject undeclared privileged instructions.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-420-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-06-R01 | Declare required capabilities | NOT RUN | — |
| BR-420-06-R02 | Derive capability set | NOT RUN | — |
| BR-420-06-R03 | Ensure instructions cannot exceed declared capabilities | NOT RUN | — |
| BR-420-06-R04 | Reject undeclared device access | NOT RUN | — |
| BR-420-06-R05 | Reject undeclared persistent-state access | NOT RUN | — |
| BR-420-06-R06 | Reject undeclared privileged instructions | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
