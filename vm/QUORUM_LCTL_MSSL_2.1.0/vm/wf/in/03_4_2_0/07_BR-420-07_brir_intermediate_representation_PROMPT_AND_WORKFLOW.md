# QUORUM BR-420-07 — BRIR intermediate representation
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **BRIR intermediate representation** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-07-R01:** Canonicalize opcode
- **BR-420-07-R02:** Canonicalize operands
- **BR-420-07-R03:** Canonicalize types
- **BR-420-07-R04:** Normalize immediates
- **BR-420-07-R05:** Make control-flow edges explicit
- **BR-420-07-R06:** Make capability dependencies explicit
- **BR-420-07-R07:** Preserve source-location provenance
- **BR-420-07-R08:** Serialize deterministically
...
Define the invariants for **BRIR intermediate representation** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-07-R01 — Canonicalize opcode.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-07-R02 — Canonicalize operands.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-420-07-R03 — Canonicalize types.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-07-R04 — Normalize immediates.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-07-R05 — Make control-flow edges explicit.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-420-07-R06 — Make capability dependencies explicit.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
7. **BR-420-07-R07 — Preserve source-location provenance.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-420-07-R08 — Serialize deterministically.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-07-R01 | Canonicalize opcode | NOT RUN | — |
| BR-420-07-R02 | Canonicalize operands | NOT RUN | — |
| BR-420-07-R03 | Canonicalize types | NOT RUN | — |
| BR-420-07-R04 | Normalize immediates | NOT RUN | — |
| BR-420-07-R05 | Make control-flow edges explicit | NOT RUN | — |
| BR-420-07-R06 | Make capability dependencies explicit | NOT RUN | — |
| BR-420-07-R07 | Preserve source-location provenance | NOT RUN | — |
| BR-420-07-R08 | Serialize deterministically | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
