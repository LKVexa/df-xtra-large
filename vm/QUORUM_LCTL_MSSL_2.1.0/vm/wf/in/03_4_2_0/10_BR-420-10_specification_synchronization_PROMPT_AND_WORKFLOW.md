# QUORUM BR-420-10 — Specification synchronization
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Specification synchronization** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-10-R01:** Synchronize BR_SPEC
- **BR-420-10-R02:** Synchronize LCTL specification
- **BR-420-10-R03:** Synchronize C header
- **BR-420-10-R04:** Synchronize opcode implementation
- **BR-420-10-R05:** Synchronize compiler tables
- **BR-420-10-R06:** Synchronize verifier tables
- **BR-420-10-R07:** Synchronize documentation
- **BR-420-10-R08:** Synchronize tests
...
Define the invariants for **Specification synchronization** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-10-R01 — Synchronize BR_SPEC.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-10-R02 — Synchronize LCTL specification.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-420-10-R03 — Synchronize C header.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-10-R04 — Synchronize opcode implementation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-420-10-R05 — Synchronize compiler tables.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-420-10-R06 — Synchronize verifier tables.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-420-10-R07 — Synchronize documentation.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-420-10-R08 — Synchronize tests.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-420-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-10-R01 | Synchronize BR_SPEC | NOT RUN | — |
| BR-420-10-R02 | Synchronize LCTL specification | NOT RUN | — |
| BR-420-10-R03 | Synchronize C header | NOT RUN | — |
| BR-420-10-R04 | Synchronize opcode implementation | NOT RUN | — |
| BR-420-10-R05 | Synchronize compiler tables | NOT RUN | — |
| BR-420-10-R06 | Synchronize verifier tables | NOT RUN | — |
| BR-420-10-R07 | Synchronize documentation | NOT RUN | — |
| BR-420-10-R08 | Synchronize tests | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
