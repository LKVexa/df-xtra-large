# QUORUM BR-420-05 — Control-flow verification
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Control-flow verification** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-420-05-R01:** Construct CFG
- **BR-420-05-R02:** Validate branches
- **BR-420-05-R03:** Validate loops
- **BR-420-05-R04:** Validate calls
- **BR-420-05-R05:** Validate return edges
- **BR-420-05-R06:** Prevent branch into malformed instruction regions
- **BR-420-05-R07:** Validate termination/yield policy
...
Define the invariants for **Control-flow verification** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-420-05-R01 — Construct CFG.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-420-05-R02 — Validate branches.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-420-05-R03 — Validate loops.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-05-R04 — Validate calls.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-05-R05 — Validate return edges.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-420-05-R06 — Prevent branch into malformed instruction regions.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
7. **BR-420-05-R07 — Validate termination/yield policy.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-05-R01 | Construct CFG | NOT RUN | — |
| BR-420-05-R02 | Validate branches | NOT RUN | — |
| BR-420-05-R03 | Validate loops | NOT RUN | — |
| BR-420-05-R04 | Validate calls | NOT RUN | — |
| BR-420-05-R05 | Validate return edges | NOT RUN | — |
| BR-420-05-R06 | Prevent branch into malformed instruction regions | NOT RUN | — |
| BR-420-05-R07 | Validate termination/yield policy | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
