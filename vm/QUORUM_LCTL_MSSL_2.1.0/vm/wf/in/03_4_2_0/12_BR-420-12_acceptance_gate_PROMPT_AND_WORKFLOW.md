# QUORUM BR-420-12 — Acceptance gate
...
### Atomic requirements
- **BR-420-12-R01:** Every production BRIM originates from verified LCTL
- **BR-420-12-R02:** Eliminate hidden semantic metadata interpretation
- **BR-420-12-R03:** Invalid programs fail before image creation
- **BR-420-12-R04:** Compiler and verifier agree
- **BR-420-12-R05:** Independent verifier validates generated BRIM
...
### Q2 — Implementation
1. **BR-420-12-R01 — Every production BRIM originates from verified LCTL.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-420-12-R02 — Eliminate hidden semantic metadata interpretation.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-420-12-R03 — Invalid programs fail before image creation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-420-12-R04 — Compiler and verifier agree.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-420-12-R05 — Independent verifier validates generated BRIM.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-420-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-420-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-420-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-420-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-420-12-R01 | Every production BRIM originates from verified LCTL | NOT RUN | — |
| BR-420-12-R02 | Eliminate hidden semantic metadata interpretation | NOT RUN | — |
| BR-420-12-R03 | Invalid programs fail before image creation | NOT RUN | — |
| BR-420-12-R04 | Compiler and verifier agree | NOT RUN | — |
| BR-420-12-R05 | Independent verifier validates generated BRIM | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-420-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
