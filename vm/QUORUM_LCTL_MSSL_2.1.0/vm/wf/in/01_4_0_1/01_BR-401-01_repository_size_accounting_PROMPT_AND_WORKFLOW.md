# QUORUM BR-401-01 — Repository-size accounting
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Repository-size accounting** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-401-01-R01:** Define source-repository size
- **BR-401-01-R02:** Define packed ZIP size
- **BR-401-01-R03:** Define stripped executable size
- **BR-401-01-R04:** Define debug executable size
- **BR-401-01-R05:** Define BRIM image size
- **BR-401-01-R06:** Define LCTL source size
- **BR-401-01-R07:** Define generated-evidence size
- **BR-401-01-R08:** Stop conflating these measurements
- **BR-401-01-R09:** Generate measurements automatically
- **BR-401-01-R10:** Fail CI when limits are exceeded
...
Define the invariants for **Repository-size accounting** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-01-R01 — Define source-repository size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-401-01-R02 — Define packed ZIP size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-401-01-R03 — Define stripped executable size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-401-01-R04 — Define debug executable size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-401-01-R05 — Define BRIM image size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-401-01-R06 — Define LCTL source size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-401-01-R07 — Define generated-evidence size.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-401-01-R08 — Stop conflating these measurements.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
9. **BR-401-01-R09 — Generate measurements automatically.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
10. **BR-401-01-R10 — Fail CI when limits are exceeded.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-401-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-01-R01 | Define source-repository size | NOT RUN | — |
| BR-401-01-R02 | Define packed ZIP size | NOT RUN | — |
| BR-401-01-R03 | Define stripped executable size | NOT RUN | — |
| BR-401-01-R04 | Define debug executable size | NOT RUN | — |
| BR-401-01-R05 | Define BRIM image size | NOT RUN | — |
| BR-401-01-R06 | Define LCTL source size | NOT RUN | — |
| BR-401-01-R07 | Define generated-evidence size | NOT RUN | — |
| BR-401-01-R08 | Stop conflating these measurements | NOT RUN | — |
| BR-401-01-R09 | Generate measurements automatically | NOT RUN | — |
| BR-401-01-R10 | Fail CI when limits are exceeded | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-01**. The next package may consume this package only when its declared prerequisite status is satisfied.

## Dependencies
- Baseline BOTTLE ROCKET 4.0.0 only
