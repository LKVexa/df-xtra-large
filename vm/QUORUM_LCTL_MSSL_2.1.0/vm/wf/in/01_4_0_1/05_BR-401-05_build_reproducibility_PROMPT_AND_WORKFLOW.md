# QUORUM BR-401-05 — Build reproducibility
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Build reproducibility** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `deploy/BOTTLE_ROCKET_SIM_CORE_4.0.0.brimg`
- `evidence/`

### Atomic requirements
- **BR-401-05-R01:** Create clean-build script
- **BR-401-05-R02:** Make compiler flags reproducible
- **BR-401-05-R03:** Freeze build configuration
- **BR-401-05-R04:** Capture toolchain versions
- **BR-401-05-R05:** Capture dependency versions
- **BR-401-05-R06:** Use deterministic timestamps
- **BR-401-05-R07:** Use deterministic archive ordering
- **BR-401-05-R08:** Require deterministic BRIM generation
- **BR-401-05-R09:** Hash all build artifacts
- **BR-401-05-R10:** Compare clean-room rebuild outputs
...
Define the invariants for **Build reproducibility** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-05-R01 — Create clean-build script.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-401-05-R02 — Make compiler flags reproducible.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-401-05-R03 — Freeze build configuration.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
4. **BR-401-05-R04 — Capture toolchain versions.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-401-05-R05 — Capture dependency versions.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-401-05-R06 — Use deterministic timestamps.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
7. **BR-401-05-R07 — Use deterministic archive ordering.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
8. **BR-401-05-R08 — Require deterministic BRIM generation.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
9. **BR-401-05-R09 — Hash all build artifacts.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
10. **BR-401-05-R10 — Compare clean-room rebuild outputs.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-401-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-05-R01 | Create clean-build script | NOT RUN | — |
| BR-401-05-R02 | Make compiler flags reproducible | NOT RUN | — |
| BR-401-05-R03 | Freeze build configuration | NOT RUN | — |
| BR-401-05-R04 | Capture toolchain versions | NOT RUN | — |
| BR-401-05-R05 | Capture dependency versions | NOT RUN | — |
| BR-401-05-R06 | Use deterministic timestamps | NOT RUN | — |
| BR-401-05-R07 | Use deterministic archive ordering | NOT RUN | — |
| BR-401-05-R08 | Require deterministic BRIM generation | NOT RUN | — |
| BR-401-05-R09 | Hash all build artifacts | NOT RUN | — |
| BR-401-05-R10 | Compare clean-room rebuild outputs | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
