# QUORUM BR-401-09 — Acceptance gate
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Acceptance gate** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-401-09-R01:** Require zero unexplained resource measurements
- **BR-401-09-R02:** Require reproducible clean builds
- **BR-401-09-R03:** Require accurate RAM figures
- **BR-401-09-R04:** Require accurate binary figures
- **BR-401-09-R05:** Require mandatory verification
- **BR-401-09-R06:** Require automatically generated evidence
- **BR-401-09-R07:** Prohibit unsupported PASS assertions
...
Define the invariants for **Acceptance gate** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-09-R01 — Require zero unexplained resource measurements.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
2. **BR-401-09-R02 — Require reproducible clean builds.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
3. **BR-401-09-R03 — Require accurate RAM figures.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
4. **BR-401-09-R04 — Require accurate binary figures.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
5. **BR-401-09-R05 — Require mandatory verification.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
6. **BR-401-09-R06 — Require automatically generated evidence.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
7. **BR-401-09-R07 — Prohibit unsupported PASS assertions.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-401-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-09-R01 | Require zero unexplained resource measurements | NOT RUN | — |
| BR-401-09-R02 | Require reproducible clean builds | NOT RUN | — |
| BR-401-09-R03 | Require accurate RAM figures | NOT RUN | — |
| BR-401-09-R04 | Require accurate binary figures | NOT RUN | — |
| BR-401-09-R05 | Require mandatory verification | NOT RUN | — |
| BR-401-09-R06 | Require automatically generated evidence | NOT RUN | — |
| BR-401-09-R07 | Prohibit unsupported PASS assertions | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
