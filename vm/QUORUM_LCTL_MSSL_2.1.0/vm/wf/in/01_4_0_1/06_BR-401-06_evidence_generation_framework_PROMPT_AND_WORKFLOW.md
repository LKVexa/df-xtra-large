# QUORUM BR-401-06 — Evidence-generation framework
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Evidence-generation framework** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-401-06-R01:** Replace manually maintained status claims
- **BR-401-06-R02:** Generate evidence directly from tests
- **BR-401-06-R03:** Generate machine-readable JSON
- **BR-401-06-R04:** Generate human-readable Markdown
- **BR-401-06-R05:** Attach hashes to evidence
- **BR-401-06-R06:** Timestamp qualification runs
- **BR-401-06-R07:** Record tool versions
- **BR-401-06-R08:** Record platform
- **BR-401-06-R09:** Record compiler
- **BR-401-06-R10:** Record test command
- **BR-401-06-R11:** Record exit status
- **BR-401-06-R12:** Record output digest
...
Define the invariants for **Evidence-generation framework** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-06-R01 — Replace manually maintained status claims.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-401-06-R02 — Generate evidence directly from tests.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-401-06-R03 — Generate machine-readable JSON.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-401-06-R04 — Generate human-readable Markdown.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-401-06-R05 — Attach hashes to evidence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-401-06-R06 — Timestamp qualification runs.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-401-06-R07 — Record tool versions.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-401-06-R08 — Record platform.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
9. **BR-401-06-R09 — Record compiler.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
10. **BR-401-06-R10 — Record test command.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
11. **BR-401-06-R11 — Record exit status.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
12. **BR-401-06-R12 — Record output digest.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-401-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-06-R01 | Replace manually maintained status claims | NOT RUN | — |
| BR-401-06-R02 | Generate evidence directly from tests | NOT RUN | — |
| BR-401-06-R03 | Generate machine-readable JSON | NOT RUN | — |
| BR-401-06-R04 | Generate human-readable Markdown | NOT RUN | — |
| BR-401-06-R05 | Attach hashes to evidence | NOT RUN | — |
| BR-401-06-R06 | Timestamp qualification runs | NOT RUN | — |
| BR-401-06-R07 | Record tool versions | NOT RUN | — |
| BR-401-06-R08 | Record platform | NOT RUN | — |
| BR-401-06-R09 | Record compiler | NOT RUN | — |
| BR-401-06-R10 | Record test command | NOT RUN | — |
| BR-401-06-R11 | Record exit status | NOT RUN | — |
| BR-401-06-R12 | Record output digest | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
