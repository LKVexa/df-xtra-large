# QUORUM BR-401-03 — Wide-word accounting
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Wide-word accounting** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-401-03-R01:** Verify exact bytes per 1,048,576-bit word
- **BR-401-03-R02:** Verify register count times word size
- **BR-401-03-R03:** Verify stack depth times word size
- **BR-401-03-R04:** Verify temporary-word count
- **BR-401-03-R05:** Verify alignment overhead
- **BR-401-03-R06:** Verify allocator overhead
- **BR-401-03-R07:** Verify serialized versus in-memory size
...
Define the invariants for **Wide-word accounting** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-03-R01 — Verify exact bytes per 1,048,576-bit word.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-401-03-R02 — Verify register count times word size.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-401-03-R03 — Verify stack depth times word size.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-401-03-R04 — Verify temporary-word count.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-401-03-R05 — Verify alignment overhead.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-401-03-R06 — Verify allocator overhead.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-401-03-R07 — Verify serialized versus in-memory size.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-401-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-03-R01 | Verify exact bytes per 1,048,576-bit word | NOT RUN | — |
| BR-401-03-R02 | Verify register count times word size | NOT RUN | — |
| BR-401-03-R03 | Verify stack depth times word size | NOT RUN | — |
| BR-401-03-R04 | Verify temporary-word count | NOT RUN | — |
| BR-401-03-R05 | Verify alignment overhead | NOT RUN | — |
| BR-401-03-R06 | Verify allocator overhead | NOT RUN | — |
| BR-401-03-R07 | Verify serialized versus in-memory size | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
