# QUORUM BR-401-08 — Operational ledger
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Operational ledger** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-401-08-R01:** Track Component ID
- **BR-401-08-R02:** Track Requirement ID
- **BR-401-08-R03:** Track implementation state
- **BR-401-08-R04:** Track test state
- **BR-401-08-R05:** Track evidence file
- **BR-401-08-R06:** Track evidence hash
- **BR-401-08-R07:** Track blocker
- **BR-401-08-R08:** Track owner
- **BR-401-08-R09:** Track dependency
- **BR-401-08-R10:** Track regression state
- **BR-401-08-R11:** Emit operational/non-operational decision
...
Define the invariants for **Operational ledger** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-08-R01 — Track Component ID.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
2. **BR-401-08-R02 — Track Requirement ID.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-401-08-R03 — Track implementation state.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-401-08-R04 — Track test state.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-401-08-R05 — Track evidence file.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-401-08-R06 — Track evidence hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
7. **BR-401-08-R07 — Track blocker.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-401-08-R08 — Track owner.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
9. **BR-401-08-R09 — Track dependency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
10. **BR-401-08-R10 — Track regression state.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
11. **BR-401-08-R11 — Emit operational/non-operational decision.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-401-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-08-R01 | Track Component ID | NOT RUN | — |
| BR-401-08-R02 | Track Requirement ID | NOT RUN | — |
| BR-401-08-R03 | Track implementation state | NOT RUN | — |
| BR-401-08-R04 | Track test state | NOT RUN | — |
| BR-401-08-R05 | Track evidence file | NOT RUN | — |
| BR-401-08-R06 | Track evidence hash | NOT RUN | — |
| BR-401-08-R07 | Track blocker | NOT RUN | — |
| BR-401-08-R08 | Track owner | NOT RUN | — |
| BR-401-08-R09 | Track dependency | NOT RUN | — |
| BR-401-08-R10 | Track regression state | NOT RUN | — |
| BR-401-08-R11 | Emit operational/non-operational decision | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
