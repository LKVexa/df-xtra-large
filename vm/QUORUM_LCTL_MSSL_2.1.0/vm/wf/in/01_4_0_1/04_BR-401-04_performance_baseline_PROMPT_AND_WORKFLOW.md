# QUORUM BR-401-04 — Performance baseline
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Performance baseline** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::br_vm_run`
- `tests/test_main.c`
- `src/brvm.c::word_add/word_sub/word_mul/word_divmod`
- `src/brvm.c::arithmetic_result`
- `host/brctl.c::signimg/verifyrun`
- `src/brvm.c::verify_ed25519`
- `src/brvm.c::br_image_load`

### Atomic requirements
- **BR-401-04-R01:** Measure instructions per second
- **BR-401-04-R02:** Measure arithmetic operations per second
- **BR-401-04-R03:** Measure load/store latency
- **BR-401-04-R04:** Measure branch latency
- **BR-401-04-R05:** Measure signature-validation latency
- **BR-401-04-R06:** Measure BRIM loading latency
- **BR-401-04-R07:** Measure startup latency
- **BR-401-04-R08:** Measure persistence commit latency
- **BR-401-04-R09:** Measure recovery latency
- **BR-401-04-R10:** Measure deterministic replay speed
...
Define the invariants for **Performance baseline** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-401-04-R01 — Measure instructions per second.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
2. **BR-401-04-R02 — Measure arithmetic operations per second.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-401-04-R03 — Measure load/store latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-401-04-R04 — Measure branch latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-401-04-R05 — Measure signature-validation latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-401-04-R06 — Measure BRIM loading latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
7. **BR-401-04-R07 — Measure startup latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-401-04-R08 — Measure persistence commit latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
9. **BR-401-04-R09 — Measure recovery latency.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
10. **BR-401-04-R10 — Measure deterministic replay speed.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-401-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-401-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-401-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-401-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-401-04-R01 | Measure instructions per second | NOT RUN | — |
| BR-401-04-R02 | Measure arithmetic operations per second | NOT RUN | — |
| BR-401-04-R03 | Measure load/store latency | NOT RUN | — |
| BR-401-04-R04 | Measure branch latency | NOT RUN | — |
| BR-401-04-R05 | Measure signature-validation latency | NOT RUN | — |
| BR-401-04-R06 | Measure BRIM loading latency | NOT RUN | — |
| BR-401-04-R07 | Measure startup latency | NOT RUN | — |
| BR-401-04-R08 | Measure persistence commit latency | NOT RUN | — |
| BR-401-04-R09 | Measure recovery latency | NOT RUN | — |
| BR-401-04-R10 | Measure deterministic replay speed | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-401-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
