# QUORUM BR-410-05 — Execution context
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Execution context** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `host/brctl.c`
- `src/brvm.h::br_apdu_command`

### Atomic requirements
- **BR-410-05-R01:** Use explicit VM instance
- **BR-410-05-R02:** Eliminate hidden global VM
- **BR-410-05-R03:** Support multiple independent instances
- **BR-410-05-R04:** Isolate instances
- **BR-410-05-R05:** Use per-instance storage context
- **BR-410-05-R06:** Use per-instance device table
- **BR-410-05-R07:** Use per-instance capabilities
- **BR-410-05-R08:** Use per-instance counters
...
Define the invariants for **Execution context** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-410-05-R01 — Use explicit VM instance.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-410-05-R02 — Eliminate hidden global VM.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-410-05-R03 — Support multiple independent instances.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-410-05-R04 — Isolate instances.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-410-05-R05 — Use per-instance storage context.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-410-05-R06 — Use per-instance device table.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
7. **BR-410-05-R07 — Use per-instance capabilities.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
8. **BR-410-05-R08 — Use per-instance counters.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-410-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-05-R01 | Use explicit VM instance | NOT RUN | — |
| BR-410-05-R02 | Eliminate hidden global VM | NOT RUN | — |
| BR-410-05-R03 | Support multiple independent instances | NOT RUN | — |
| BR-410-05-R04 | Isolate instances | NOT RUN | — |
| BR-410-05-R05 | Use per-instance storage context | NOT RUN | — |
| BR-410-05-R06 | Use per-instance device table | NOT RUN | — |
| BR-410-05-R07 | Use per-instance capabilities | NOT RUN | — |
| BR-410-05-R08 | Use per-instance counters | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
