# QUORUM BR-410-09 — Acceptance gate
...
### Atomic requirements
- **BR-410-09-R01:** brvm-core builds independently
- **BR-410-09-R02:** No host filesystem dependency in core
- **BR-410-09-R03:** Same BRIM executes identically under at least two adapters
- **BR-410-09-R04:** Demonstrate VM instance isolation
- **BR-410-09-R05:** Fully mediate host-service calls
...
### Q2 — Implementation
1. **BR-410-09-R01 — brvm-core builds independently.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-410-09-R02 — No host filesystem dependency in core.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-410-09-R03 — Same BRIM executes identically under at least two adapters.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-410-09-R04 — Demonstrate VM instance isolation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-410-09-R05 — Fully mediate host-service calls.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-410-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-09-R01 | brvm-core builds independently | NOT RUN | — |
| BR-410-09-R02 | No host filesystem dependency in core | NOT RUN | — |
| BR-410-09-R03 | Same BRIM executes identically under at least two adapters | NOT RUN | — |
| BR-410-09-R04 | Demonstrate VM instance isolation | NOT RUN | — |
| BR-410-09-R05 | Fully mediate host-service calls | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
