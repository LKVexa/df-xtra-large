# QUORUM BR-410-02 — Remove host assumptions from the core
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Remove host assumptions from the core** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::sync_parent/br_vm_save/image_slot_write`

### Atomic requirements
- **BR-410-02-R01:** Remove direct stdio
- **BR-410-02-R02:** Remove direct filesystem calls
- **BR-410-02-R03:** Remove environment-variable dependencies
- **BR-410-02-R04:** Remove terminal dependencies
- **BR-410-02-R05:** Remove process-global state
- **BR-410-02-R06:** Remove POSIX-only assumptions
- **BR-410-02-R07:** Remove direct OpenSSL dependency from core semantics
...
Define the invariants for **Remove host assumptions from the core** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-410-02-R01 — Remove direct stdio.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
2. **BR-410-02-R02 — Remove direct filesystem calls.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-410-02-R03 — Remove environment-variable dependencies.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
4. **BR-410-02-R04 — Remove terminal dependencies.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
5. **BR-410-02-R05 — Remove process-global state.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
6. **BR-410-02-R06 — Remove POSIX-only assumptions.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
7. **BR-410-02-R07 — Remove direct OpenSSL dependency from core semantics.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-410-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-02-R01 | Remove direct stdio | NOT RUN | — |
| BR-410-02-R02 | Remove direct filesystem calls | NOT RUN | — |
| BR-410-02-R03 | Remove environment-variable dependencies | NOT RUN | — |
| BR-410-02-R04 | Remove terminal dependencies | NOT RUN | — |
| BR-410-02-R05 | Remove process-global state | NOT RUN | — |
| BR-410-02-R06 | Remove POSIX-only assumptions | NOT RUN | — |
| BR-410-02-R07 | Remove direct OpenSSL dependency from core semantics | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
