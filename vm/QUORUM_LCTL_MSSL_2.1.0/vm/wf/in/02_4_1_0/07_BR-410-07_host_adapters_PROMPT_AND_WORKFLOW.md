# QUORUM BR-410-07 — Host adapters
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Host adapters** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-410-07-R01:** Implement POSIX adapter
- **BR-410-07-R02:** Implement Windows adapter
- **BR-410-07-R03:** Implement bare-metal test adapter
- **BR-410-07-R04:** Implement deterministic test adapter
- **BR-410-07-R05:** Implement in-memory storage adapter
- **BR-410-07-R06:** Implement simulated smart-card adapter
...
Define the invariants for **Host adapters** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-410-07-R01 — Implement POSIX adapter.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-410-07-R02 — Implement Windows adapter.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-410-07-R03 — Implement bare-metal test adapter.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-410-07-R04 — Implement deterministic test adapter.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-410-07-R05 — Implement in-memory storage adapter.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-410-07-R06 — Implement simulated smart-card adapter.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-410-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-07-R01 | Implement POSIX adapter | NOT RUN | — |
| BR-410-07-R02 | Implement Windows adapter | NOT RUN | — |
| BR-410-07-R03 | Implement bare-metal test adapter | NOT RUN | — |
| BR-410-07-R04 | Implement deterministic test adapter | NOT RUN | — |
| BR-410-07-R05 | Implement in-memory storage adapter | NOT RUN | — |
| BR-410-07-R06 | Implement simulated smart-card adapter | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
