# QUORUM BR-410-01 — Create brvm-core
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Create brvm-core** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-410-01-R01:** Extract ISA executor
- **BR-410-01-R02:** Extract register engine
- **BR-410-01-R03:** Extract arithmetic engine
- **BR-410-01-R04:** Extract stack engine
- **BR-410-01-R05:** Extract memory engine
- **BR-410-01-R06:** Extract trap engine
- **BR-410-01-R07:** Extract scheduler
- **BR-410-01-R08:** Extract capability engine
- **BR-410-01-R09:** Extract image loader
- **BR-410-01-R10:** Extract VM state machine
...
Define the invariants for **Create brvm-core** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-410-01-R01 — Extract ISA executor.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-410-01-R02 — Extract register engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-410-01-R03 — Extract arithmetic engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-410-01-R04 — Extract stack engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-410-01-R05 — Extract memory engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-410-01-R06 — Extract trap engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-410-01-R07 — Extract scheduler.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-410-01-R08 — Extract capability engine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
9. **BR-410-01-R09 — Extract image loader.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
10. **BR-410-01-R10 — Extract VM state machine.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-410-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-01-R01 | Extract ISA executor | NOT RUN | — |
| BR-410-01-R02 | Extract register engine | NOT RUN | — |
| BR-410-01-R03 | Extract arithmetic engine | NOT RUN | — |
| BR-410-01-R04 | Extract stack engine | NOT RUN | — |
| BR-410-01-R05 | Extract memory engine | NOT RUN | — |
| BR-410-01-R06 | Extract trap engine | NOT RUN | — |
| BR-410-01-R07 | Extract scheduler | NOT RUN | — |
| BR-410-01-R08 | Extract capability engine | NOT RUN | — |
| BR-410-01-R09 | Extract image loader | NOT RUN | — |
| BR-410-01-R10 | Extract VM state machine | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
