...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Error model** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-410-06-R01:** Define core status codes
- **BR-410-06-R02:** Define trap status codes
- **BR-410-06-R03:** Define loader status codes
- **BR-410-06-R04:** Define storage status codes
- **BR-410-06-R05:** Define signature status codes
- **BR-410-06-R06:** Define capability violations
- **BR-410-06-R07:** Define resource exhaustion
- **BR-410-06-R08:** Define malformed image
- **BR-410-06-R09:** Define unsupported ISA
- **BR-410-06-R10:** Define unsupported ABI
...
Define the invariants for **Error model** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-410-06-R01 — Define core status codes.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-410-06-R02 — Define trap status codes.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-410-06-R03 — Define loader status codes.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-410-06-R04 — Define storage status codes.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-410-06-R05 — Define signature status codes.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-410-06-R06 — Define capability violations.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-410-06-R07 — Define resource exhaustion.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-410-06-R08 — Define malformed image.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
9. **BR-410-06-R09 — Define unsupported ISA.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
10. **BR-410-06-R10 — Define unsupported ABI.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-410-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-410-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-410-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-410-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-410-06-R01 | Define core status codes | NOT RUN | — |
| BR-410-06-R02 | Define trap status codes | NOT RUN | — |
| BR-410-06-R03 | Define loader status codes | NOT RUN | — |
| BR-410-06-R04 | Define storage status codes | NOT RUN | — |
| BR-410-06-R05 | Define signature status codes | NOT RUN | — |
| BR-410-06-R06 | Define capability violations | NOT RUN | — |
| BR-410-06-R07 | Define resource exhaustion | NOT RUN | — |
| BR-410-06-R08 | Define malformed image | NOT RUN | — |
| BR-410-06-R09 | Define unsupported ISA | NOT RUN | — |
| BR-410-06-R10 | Define unsupported ABI | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-410-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
