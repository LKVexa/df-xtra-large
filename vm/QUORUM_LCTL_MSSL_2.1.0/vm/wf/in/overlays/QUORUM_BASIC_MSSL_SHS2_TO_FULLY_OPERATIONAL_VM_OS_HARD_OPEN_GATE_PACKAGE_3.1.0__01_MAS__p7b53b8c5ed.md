...
## HARD-OPEN execution mode — mandatory

Apply `03_HARD_OPEN_GATE_PROTOCOL.md` and `04_HARD_OPEN_GATE_CONTROLLER.md` to every phase of this master workflow.

A blocked builder-addressable predicate is an **instruction to remediate**, not a stopping point. Recursively open dependencies, implement missing native authority, repair failing code, rebuild, run the required native/negative/fault tests, regenerate current-root evidence, and retry the gate immediately.

Do not weaken any original gate. HARD_OPEN changes execution behavior from passive audit to active closure; it does not change the acceptance standard.

For genuinely independent/external predicates, complete all builder-side preparation and emit a handoff-ready bundle, but leave the external predicate false until legitimate external evidence is returned.
