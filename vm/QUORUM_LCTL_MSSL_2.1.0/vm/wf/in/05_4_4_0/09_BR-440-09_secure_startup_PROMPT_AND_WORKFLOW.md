# QUORUM BR-440-09 — Secure startup
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Secure startup** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::br_vm_recover`
- `host/brctl.c::update`

### Atomic requirements
- **BR-440-09-R01:** Initialize immutable trust configuration
- **BR-440-09-R02:** Recover persistence
- **BR-440-09-R03:** Validate control record
- **BR-440-09-R04:** Select candidate image
- **BR-440-09-R05:** Verify image
- **BR-440-09-R06:** Verify rollback state
- **BR-440-09-R07:** Derive capabilities
- **BR-440-09-R08:** Initialize VM
- **BR-440-09-R09:** Execute
...
Define the invariants for **Secure startup** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-09-R01 — Initialize immutable trust configuration.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-440-09-R02 — Recover persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-440-09-R03 — Validate control record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-440-09-R04 — Select candidate image.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-440-09-R05 — Verify image.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-440-09-R06 — Verify rollback state.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-440-09-R07 — Derive capabilities.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
8. **BR-440-09-R08 — Initialize VM.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
9. **BR-440-09-R09 — Execute.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-440-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-09-R01 | Initialize immutable trust configuration | NOT RUN | — |
| BR-440-09-R02 | Recover persistence | NOT RUN | — |
| BR-440-09-R03 | Validate control record | NOT RUN | — |
| BR-440-09-R04 | Select candidate image | NOT RUN | — |
| BR-440-09-R05 | Verify image | NOT RUN | — |
| BR-440-09-R06 | Verify rollback state | NOT RUN | — |
| BR-440-09-R07 | Derive capabilities | NOT RUN | — |
| BR-440-09-R08 | Initialize VM | NOT RUN | — |
| BR-440-09-R09 | Execute | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
