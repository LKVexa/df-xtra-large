...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Key rotation** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-06-R01:** Authorize new issuer
- **BR-440-06-R02:** Deauthorize old issuer
- **BR-440-06-R03:** Define overlap period
- **BR-440-06-R04:** Use monotonic key epoch
- **BR-440-06-R05:** Use signed rotation manifest
...
Define the invariants for **Key rotation** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-06-R01 — Authorize new issuer.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-440-06-R02 — Deauthorize old issuer.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-440-06-R03 — Define overlap period.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-440-06-R04 — Use monotonic key epoch.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-440-06-R05 — Use signed rotation manifest.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-440-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-06-R01 | Authorize new issuer | NOT RUN | — |
| BR-440-06-R02 | Deauthorize old issuer | NOT RUN | — |
| BR-440-06-R03 | Define overlap period | NOT RUN | — |
| BR-440-06-R04 | Use monotonic key epoch | NOT RUN | — |
| BR-440-06-R05 | Use signed rotation manifest | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
