# QUORUM BR-440-11 — Acceptance gate
...
### Atomic requirements
- **BR-440-11-R01:** Production VM cannot execute unsigned code
- **BR-440-11-R02:** Rollback attacks fail
- **BR-440-11-R03:** Key rotation works
- **BR-440-11-R04:** Revoked signing authority fails
- **BR-440-11-R05:** Development bypasses absent from production build
...
### Q2 — Implementation
1. **BR-440-11-R01 — Production VM cannot execute unsigned code.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-440-11-R02 — Rollback attacks fail.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-440-11-R03 — Key rotation works.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-440-11-R04 — Revoked signing authority fails.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-440-11-R05 — Development bypasses absent from production build.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-440-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-11-R01 | Production VM cannot execute unsigned code | NOT RUN | — |
| BR-440-11-R02 | Rollback attacks fail | NOT RUN | — |
| BR-440-11-R03 | Key rotation works | NOT RUN | — |
| BR-440-11-R04 | Revoked signing authority fails | NOT RUN | — |
| BR-440-11-R05 | Development bypasses absent from production build | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
