...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Trust root** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-01-R01:** Define immutable root public key
- **BR-440-01-R02:** Define root-key identifier
- **BR-440-01-R03:** Define trust-policy version
- **BR-440-01-R04:** Support optional hardware trust anchor
- **BR-440-01-R05:** Separate development keys
...
Define the invariants for **Trust root** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-01-R01 — Define immutable root public key.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-440-01-R02 — Define root-key identifier.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-440-01-R03 — Define trust-policy version.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-440-01-R04 — Support optional hardware trust anchor.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-440-01-R05 — Separate development keys.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-440-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-01-R01 | Define immutable root public key | NOT RUN | — |
| BR-440-01-R02 | Define root-key identifier | NOT RUN | — |
| BR-440-01-R03 | Define trust-policy version | NOT RUN | — |
| BR-440-01-R04 | Support optional hardware trust anchor | NOT RUN | — |
| BR-440-01-R05 | Separate development keys | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
