# QUORUM BR-440-08 — Rollback protection
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Rollback protection** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.h::persistent_generation/last_txid`
- `src/brvm.c::br_vm_save/br_vm_recover`

### Atomic requirements
- **BR-440-08-R01:** Use monotonic version
- **BR-440-08-R02:** Use monotonic generation
- **BR-440-08-R03:** Use issuer epoch
- **BR-440-08-R04:** Use transaction sequence
- **BR-440-08-R05:** Reject stale state
- **BR-440-08-R06:** Reject stale control record
...
Define the invariants for **Rollback protection** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-08-R01 — Use monotonic version.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-440-08-R02 — Use monotonic generation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-440-08-R03 — Use issuer epoch.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-440-08-R04 — Use transaction sequence.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-440-08-R05 — Reject stale state.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
6. **BR-440-08-R06 — Reject stale control record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-440-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-08-R01 | Use monotonic version | NOT RUN | — |
| BR-440-08-R02 | Use monotonic generation | NOT RUN | — |
| BR-440-08-R03 | Use issuer epoch | NOT RUN | — |
| BR-440-08-R04 | Use transaction sequence | NOT RUN | — |
| BR-440-08-R05 | Reject stale state | NOT RUN | — |
| BR-440-08-R06 | Reject stale control record | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
