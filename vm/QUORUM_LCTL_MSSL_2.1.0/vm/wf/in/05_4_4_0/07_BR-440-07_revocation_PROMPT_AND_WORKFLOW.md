...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Revocation** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-07-R01:** Define revoked key list
- **BR-440-07-R02:** Define revoked image versions
- **BR-440-07-R03:** Define compromised issuer response
- **BR-440-07-R04:** Define emergency recovery
- **BR-440-07-R05:** Define offline revocation model
...
Define the invariants for **Revocation** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-07-R01 — Define revoked key list.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-440-07-R02 — Define revoked image versions.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-440-07-R03 — Define compromised issuer response.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-440-07-R04 — Define emergency recovery.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-440-07-R05 — Define offline revocation model.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-440-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-07-R01 | Define revoked key list | NOT RUN | — |
| BR-440-07-R02 | Define revoked image versions | NOT RUN | — |
| BR-440-07-R03 | Define compromised issuer response | NOT RUN | — |
| BR-440-07-R04 | Define emergency recovery | NOT RUN | — |
| BR-440-07-R05 | Define offline revocation model | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
