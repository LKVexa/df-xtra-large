# QUORUM BR-440-04 — Production-loader policy
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Production-loader policy** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-04-R01:** Disable unsigned run
- **BR-440-04-R02:** Disable raw executable APDU load
- **BR-440-04-R03:** Disable arbitrary opcode injection
- **BR-440-04-R04:** Disable bypass flags
- **BR-440-04-R05:** Compile-time separate development pathways
- **BR-440-04-R06:** Mark development images unmistakably
...
Define the invariants for **Production-loader policy** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-04-R01 — Disable unsigned run.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
2. **BR-440-04-R02 — Disable raw executable APDU load.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-440-04-R03 — Disable arbitrary opcode injection.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
4. **BR-440-04-R04 — Disable bypass flags.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
5. **BR-440-04-R05 — Compile-time separate development pathways.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-440-04-R06 — Mark development images unmistakably.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-440-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-04-R01 | Disable unsigned run | NOT RUN | — |
| BR-440-04-R02 | Disable raw executable APDU load | NOT RUN | — |
| BR-440-04-R03 | Disable arbitrary opcode injection | NOT RUN | — |
| BR-440-04-R04 | Disable bypass flags | NOT RUN | — |
| BR-440-04-R05 | Compile-time separate development pathways | NOT RUN | — |
| BR-440-04-R06 | Mark development images unmistakably | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
