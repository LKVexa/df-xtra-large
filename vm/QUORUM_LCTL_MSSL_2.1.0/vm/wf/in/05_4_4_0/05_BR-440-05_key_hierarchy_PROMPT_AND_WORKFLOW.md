# QUORUM BR-440-05 — Key hierarchy
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Key hierarchy** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-05-R01:** Define root key
- **BR-440-05-R02:** Define issuer keys
- **BR-440-05-R03:** Define release keys
- **BR-440-05-R04:** Define recovery key
- **BR-440-05-R05:** Define device-specific key if required
- **BR-440-05-R06:** Define test keys
...
Define the invariants for **Key hierarchy** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-05-R01 — Define root key.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-440-05-R02 — Define issuer keys.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-440-05-R03 — Define release keys.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-440-05-R04 — Define recovery key.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-440-05-R05 — Define device-specific key if required.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-440-05-R06 — Define test keys.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-440-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-05-R01 | Define root key | NOT RUN | — |
| BR-440-05-R02 | Define issuer keys | NOT RUN | — |
| BR-440-05-R03 | Define release keys | NOT RUN | — |
| BR-440-05-R04 | Define recovery key | NOT RUN | — |
| BR-440-05-R05 | Define device-specific key if required | NOT RUN | — |
| BR-440-05-R06 | Define test keys | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
