# QUORUM BR-440-10 — Security logging
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Security logging** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-440-10-R01:** Log signature accepted
- **BR-440-10-R02:** Log signature rejected
- **BR-440-10-R03:** Log rollback rejected
- **BR-440-10-R04:** Log revoked issuer
- **BR-440-10-R05:** Log malformed image
- **BR-440-10-R06:** Log unauthorized capability
- **BR-440-10-R07:** Log recovery operation
...
Define the invariants for **Security logging** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-440-10-R01 — Log signature accepted.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-440-10-R02 — Log signature rejected.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
3. **BR-440-10-R03 — Log rollback rejected.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
4. **BR-440-10-R04 — Log revoked issuer.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-440-10-R05 — Log malformed image.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-440-10-R06 — Log unauthorized capability.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-440-10-R07 — Log recovery operation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-440-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-440-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-440-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-440-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-440-10-R01 | Log signature accepted | NOT RUN | — |
| BR-440-10-R02 | Log signature rejected | NOT RUN | — |
| BR-440-10-R03 | Log rollback rejected | NOT RUN | — |
| BR-440-10-R04 | Log revoked issuer | NOT RUN | — |
| BR-440-10-R05 | Log malformed image | NOT RUN | — |
| BR-440-10-R06 | Log unauthorized capability | NOT RUN | — |
| BR-440-10-R07 | Log recovery operation | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-440-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
