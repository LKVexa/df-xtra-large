# QUORUM BR-450-03 — Control record
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Control record** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-450-03-R01:** Define magic
- **BR-450-03-R02:** Define format version
- **BR-450-03-R03:** Define active slot
- **BR-450-03-R04:** Define generation
- **BR-450-03-R05:** Define transaction sequence
- **BR-450-03-R06:** Define issuer epoch
- **BR-450-03-R07:** Define image hash
- **BR-450-03-R08:** Define authentication tag/signature
- **BR-450-03-R09:** Define recovery status
...
Define the invariants for **Control record** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-03-R01 — Define magic.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-450-03-R02 — Define format version.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-450-03-R03 — Define active slot.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-450-03-R04 — Define generation.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-450-03-R05 — Define transaction sequence.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-450-03-R06 — Define issuer epoch.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-450-03-R07 — Define image hash.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-450-03-R08 — Define authentication tag/signature.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
9. **BR-450-03-R09 — Define recovery status.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-450-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-03-R01 | Define magic | NOT RUN | — |
| BR-450-03-R02 | Define format version | NOT RUN | — |
| BR-450-03-R03 | Define active slot | NOT RUN | — |
| BR-450-03-R04 | Define generation | NOT RUN | — |
| BR-450-03-R05 | Define transaction sequence | NOT RUN | — |
| BR-450-03-R06 | Define issuer epoch | NOT RUN | — |
| BR-450-03-R07 | Define image hash | NOT RUN | — |
| BR-450-03-R08 | Define authentication tag/signature | NOT RUN | — |
| BR-450-03-R09 | Define recovery status | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
