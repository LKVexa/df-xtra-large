# QUORUM BR-450-10 — Persistent VM data
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Persistent VM data** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-450-10-R01:** Separate executable state from guest data
- **BR-450-10-R02:** Namespace persistent objects
- **BR-450-10-R03:** Enforce quotas
- **BR-450-10-R04:** Version persistent data
- **BR-450-10-R05:** Protect integrity
- **BR-450-10-R06:** Support migration
...
Define the invariants for **Persistent VM data** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-10-R01 — Separate executable state from guest data.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-450-10-R02 — Namespace persistent objects.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-450-10-R03 — Enforce quotas.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-450-10-R04 — Version persistent data.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-450-10-R05 — Protect integrity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-450-10-R06 — Support migration.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-450-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-10-R01 | Separate executable state from guest data | NOT RUN | — |
| BR-450-10-R02 | Namespace persistent objects | NOT RUN | — |
| BR-450-10-R03 | Enforce quotas | NOT RUN | — |
| BR-450-10-R04 | Version persistent data | NOT RUN | — |
| BR-450-10-R05 | Protect integrity | NOT RUN | — |
| BR-450-10-R06 | Support migration | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
