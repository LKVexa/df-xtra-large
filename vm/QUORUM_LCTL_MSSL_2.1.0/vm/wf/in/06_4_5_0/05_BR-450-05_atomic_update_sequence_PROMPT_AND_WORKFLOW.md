# QUORUM BR-450-05 — Atomic update sequence
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Atomic update sequence** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::image_slot_write`
- `host/brctl.c::update`

### Atomic requirements
- **BR-450-05-R01:** Verify incoming image
- **BR-450-05-R02:** Choose inactive slot
- **BR-450-05-R03:** Write candidate
- **BR-450-05-R04:** Sync candidate
- **BR-450-05-R05:** Reread candidate
- **BR-450-05-R06:** Verify candidate
- **BR-450-05-R07:** Write pending control record
- **BR-450-05-R08:** Sync pending record
- **BR-450-05-R09:** Atomically activate
- **BR-450-05-R10:** Sync activation
- **BR-450-05-R11:** Confirm activation
...
Define the invariants for **Atomic update sequence** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-05-R01 — Verify incoming image.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-450-05-R02 — Choose inactive slot.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-450-05-R03 — Write candidate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-450-05-R04 — Sync candidate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-450-05-R05 — Reread candidate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-450-05-R06 — Verify candidate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-450-05-R07 — Write pending control record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-450-05-R08 — Sync pending record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
9. **BR-450-05-R09 — Atomically activate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
10. **BR-450-05-R10 — Sync activation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
11. **BR-450-05-R11 — Confirm activation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-450-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-05-R01 | Verify incoming image | NOT RUN | — |
| BR-450-05-R02 | Choose inactive slot | NOT RUN | — |
| BR-450-05-R03 | Write candidate | NOT RUN | — |
| BR-450-05-R04 | Sync candidate | NOT RUN | — |
| BR-450-05-R05 | Reread candidate | NOT RUN | — |
| BR-450-05-R06 | Verify candidate | NOT RUN | — |
| BR-450-05-R07 | Write pending control record | NOT RUN | — |
| BR-450-05-R08 | Sync pending record | NOT RUN | — |
| BR-450-05-R09 | Atomically activate | NOT RUN | — |
| BR-450-05-R10 | Sync activation | NOT RUN | — |
| BR-450-05-R11 | Confirm activation | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
