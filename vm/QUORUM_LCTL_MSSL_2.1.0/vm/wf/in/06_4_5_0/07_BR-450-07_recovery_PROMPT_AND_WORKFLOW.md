...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Recovery** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::image_slot_load`
- `host/brctl.c::statecmd`

### Atomic requirements
- **BR-450-07-R01:** Detect incomplete transaction
- **BR-450-07-R02:** Validate both slots
- **BR-450-07-R03:** Select highest valid committed generation
- **BR-450-07-R04:** Reject corrupted control records
- **BR-450-07-R05:** Reject mismatched image/control pairs
- **BR-450-07-R06:** Return to last-known-good state
...
Define the invariants for **Recovery** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-07-R01 — Detect incomplete transaction.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-450-07-R02 — Validate both slots.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-450-07-R03 — Select highest valid committed generation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-450-07-R04 — Reject corrupted control records.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-450-07-R05 — Reject mismatched image/control pairs.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
6. **BR-450-07-R06 — Return to last-known-good state.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-450-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-07-R01 | Detect incomplete transaction | NOT RUN | — |
| BR-450-07-R02 | Validate both slots | NOT RUN | — |
| BR-450-07-R03 | Select highest valid committed generation | NOT RUN | — |
| BR-450-07-R04 | Reject corrupted control records | NOT RUN | — |
| BR-450-07-R05 | Reject mismatched image/control pairs | NOT RUN | — |
| BR-450-07-R06 | Return to last-known-good state | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
