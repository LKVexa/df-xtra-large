...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Concurrency** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-450-08-R01:** Implement storage locks
- **BR-450-08-R02:** Implement update locks
- **BR-450-08-R03:** Use lock files where appropriate
- **BR-450-08-R04:** Protect against multiple host processes
- **BR-450-08-R05:** Prevent partial concurrent upgrade
...
Define the invariants for **Concurrency** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-08-R01 — Implement storage locks.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-450-08-R02 — Implement update locks.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-450-08-R03 — Use lock files where appropriate.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-450-08-R04 — Protect against multiple host processes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-450-08-R05 — Prevent partial concurrent upgrade.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-450-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-08-R01 | Implement storage locks | NOT RUN | — |
| BR-450-08-R02 | Implement update locks | NOT RUN | — |
| BR-450-08-R03 | Use lock files where appropriate | NOT RUN | — |
| BR-450-08-R04 | Protect against multiple host processes | NOT RUN | — |
| BR-450-08-R05 | Prevent partial concurrent upgrade | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
