# QUORUM BR-450-09 — Filesystem hardening
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Filesystem hardening** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-450-09-R01:** Protect against symlinks
- **BR-450-09-R02:** Use restrictive permissions
- **BR-450-09-R03:** Canonicalize paths
- **BR-450-09-R04:** Use safe temporary files
- **BR-450-09-R05:** Fsync parent directory where appropriate
- **BR-450-09-R06:** Handle unexpected truncation
...
Define the invariants for **Filesystem hardening** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-09-R01 — Protect against symlinks.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-450-09-R02 — Use restrictive permissions.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-450-09-R03 — Canonicalize paths.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-450-09-R04 — Use safe temporary files.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-450-09-R05 — Fsync parent directory where appropriate.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-450-09-R06 — Handle unexpected truncation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-450-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-09-R01 | Protect against symlinks | NOT RUN | — |
| BR-450-09-R02 | Use restrictive permissions | NOT RUN | — |
| BR-450-09-R03 | Canonicalize paths | NOT RUN | — |
| BR-450-09-R04 | Use safe temporary files | NOT RUN | — |
| BR-450-09-R05 | Fsync parent directory where appropriate | NOT RUN | — |
| BR-450-09-R06 | Handle unexpected truncation | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
