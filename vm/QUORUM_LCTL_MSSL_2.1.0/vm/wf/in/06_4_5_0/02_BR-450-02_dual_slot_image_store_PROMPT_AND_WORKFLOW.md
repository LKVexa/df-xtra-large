# QUORUM BR-450-02 — Dual-slot image store
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Dual-slot image store** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::image_slot_write/image_slot_load`
- `src/brvm.c::br_vm_recover`

### Atomic requirements
- **BR-450-02-R01:** Maintain Slot A
- **BR-450-02-R02:** Maintain Slot B
- **BR-450-02-R03:** Track active slot
- **BR-450-02-R04:** Track candidate slot
- **BR-450-02-R05:** Track previous-good slot
- **BR-450-02-R06:** Track slot generation
- **BR-450-02-R07:** Track slot hash
- **BR-450-02-R08:** Track slot signature status
...
Define the invariants for **Dual-slot image store** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-450-02-R01 — Maintain Slot A.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-450-02-R02 — Maintain Slot B.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-450-02-R03 — Track active slot.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
4. **BR-450-02-R04 — Track candidate slot.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-450-02-R05 — Track previous-good slot.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
6. **BR-450-02-R06 — Track slot generation.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
7. **BR-450-02-R07 — Track slot hash.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-450-02-R08 — Track slot signature status.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-450-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-450-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-450-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-450-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-450-02-R01 | Maintain Slot A | NOT RUN | — |
| BR-450-02-R02 | Maintain Slot B | NOT RUN | — |
| BR-450-02-R03 | Track active slot | NOT RUN | — |
| BR-450-02-R04 | Track candidate slot | NOT RUN | — |
| BR-450-02-R05 | Track previous-good slot | NOT RUN | — |
| BR-450-02-R06 | Track slot generation | NOT RUN | — |
| BR-450-02-R07 | Track slot hash | NOT RUN | — |
| BR-450-02-R08 | Track slot signature status | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-450-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
