# QUORUM BR-430-15 — Acceptance gate
...
### Atomic requirements
- **BR-430-15-R01:** Freeze ISA specification
- **BR-430-15-R02:** Independently test every opcode
- **BR-430-15-R03:** Independently test every trap
- **BR-430-15-R04:** Pass ABI conformance
- **BR-430-15-R05:** Document backward-compatibility rules
...
### Q2 — Implementation
1. **BR-430-15-R01 — Freeze ISA specification.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-430-15-R02 — Independently test every opcode.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-430-15-R03 — Independently test every trap.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-430-15-R04 — Pass ABI conformance.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
5. **BR-430-15-R05 — Document backward-compatibility rules.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-15_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-15_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-15_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-15_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-15-R01 | Freeze ISA specification | NOT RUN | — |
| BR-430-15-R02 | Independently test every opcode | NOT RUN | — |
| BR-430-15-R03 | Independently test every trap | NOT RUN | — |
| BR-430-15-R04 | Pass ABI conformance | NOT RUN | — |
| BR-430-15-R05 | Document backward-compatibility rules | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-15**. The next package may consume this package only when its declared prerequisite status is satisfied.
