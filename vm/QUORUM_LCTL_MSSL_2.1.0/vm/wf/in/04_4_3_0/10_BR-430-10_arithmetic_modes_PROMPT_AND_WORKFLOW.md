# QUORUM BR-430-10 — Arithmetic modes
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Arithmetic modes** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-10-R01:** Specify unsigned semantics
- **BR-430-10-R02:** Specify signed two's complement semantics
- **BR-430-10-R03:** Specify modular semantics
- **BR-430-10-R04:** Specify saturating semantics if retained
- **BR-430-10-R05:** Specify exact/trapping mode if desired
- **BR-430-10-R06:** Define mode encoding
- **BR-430-10-R07:** Define cross-mode conversion rules
...
Define the invariants for **Arithmetic modes** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-10-R01 — Specify unsigned semantics.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-10-R02 — Specify signed two's complement semantics.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-10-R03 — Specify modular semantics.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-10-R04 — Specify saturating semantics if retained.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-10-R05 — Specify exact/trapping mode if desired.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-10-R06 — Define mode encoding.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-10-R07 — Define cross-mode conversion rules.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-10-R01 | Specify unsigned semantics | NOT RUN | — |
| BR-430-10-R02 | Specify signed two's complement semantics | NOT RUN | — |
| BR-430-10-R03 | Specify modular semantics | NOT RUN | — |
| BR-430-10-R04 | Specify saturating semantics if retained | NOT RUN | — |
| BR-430-10-R05 | Specify exact/trapping mode if desired | NOT RUN | — |
| BR-430-10-R06 | Define mode encoding | NOT RUN | — |
| BR-430-10-R07 | Define cross-mode conversion rules | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
