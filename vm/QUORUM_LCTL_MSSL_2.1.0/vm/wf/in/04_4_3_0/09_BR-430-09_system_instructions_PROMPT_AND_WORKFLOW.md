# QUORUM BR-430-09 — System instructions
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **System instructions** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-09-R01:** Specify service call
- **BR-430-09-R02:** Specify device call
- **BR-430-09-R03:** Specify capability query
- **BR-430-09-R04:** Specify trap
- **BR-430-09-R05:** Specify yield
- **BR-430-09-R06:** Specify state checkpoint
- **BR-430-09-R07:** Specify optional development debug trap
...
Define the invariants for **System instructions** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-09-R01 — Specify service call.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-09-R02 — Specify device call.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-09-R03 — Specify capability query.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-09-R04 — Specify trap.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-09-R05 — Specify yield.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-09-R06 — Specify state checkpoint.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-09-R07 — Specify optional development debug trap.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-09-R01 | Specify service call | NOT RUN | — |
| BR-430-09-R02 | Specify device call | NOT RUN | — |
| BR-430-09-R03 | Specify capability query | NOT RUN | — |
| BR-430-09-R04 | Specify trap | NOT RUN | — |
| BR-430-09-R05 | Specify yield | NOT RUN | — |
| BR-430-09-R06 | Specify state checkpoint | NOT RUN | — |
| BR-430-09-R07 | Specify optional development debug trap | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
