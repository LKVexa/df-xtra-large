# QUORUM BR-430-04 — Shift/rotation semantics
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Shift/rotation semantics** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::word_shift`
- `src/brvm.c::br_vm_step`

### Atomic requirements
- **BR-430-04-R01:** Specify SHL
- **BR-430-04-R02:** Specify SHR
- **BR-430-04-R03:** Specify arithmetic shift if needed
- **BR-430-04-R04:** Specify rotate if justified
- **BR-430-04-R05:** Specify oversized shift behavior
- **BR-430-04-R06:** Specify zero shift
- **BR-430-04-R07:** Specify width-edge semantics
...
Define the invariants for **Shift/rotation semantics** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-04-R01 — Specify SHL.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-04-R02 — Specify SHR.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-04-R03 — Specify arithmetic shift if needed.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-04-R04 — Specify rotate if justified.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-04-R05 — Specify oversized shift behavior.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-04-R06 — Specify zero shift.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-04-R07 — Specify width-edge semantics.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-04-R01 | Specify SHL | NOT RUN | — |
| BR-430-04-R02 | Specify SHR | NOT RUN | — |
| BR-430-04-R03 | Specify arithmetic shift if needed | NOT RUN | — |
| BR-430-04-R04 | Specify rotate if justified | NOT RUN | — |
| BR-430-04-R05 | Specify oversized shift behavior | NOT RUN | — |
| BR-430-04-R06 | Specify zero shift | NOT RUN | — |
| BR-430-04-R07 | Specify width-edge semantics | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
