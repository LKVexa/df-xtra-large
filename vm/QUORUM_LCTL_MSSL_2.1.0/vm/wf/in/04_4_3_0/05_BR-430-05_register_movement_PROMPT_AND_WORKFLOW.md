# QUORUM BR-430-05 — Register movement
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Register movement** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-05-R01:** Specify MOV
- **BR-430-05-R02:** Specify immediate load
- **BR-430-05-R03:** Specify constant load
- **BR-430-05-R04:** Specify register copy
- **BR-430-05-R05:** Specify clear
...
Define the invariants for **Register movement** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-05-R01 — Specify MOV.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-05-R02 — Specify immediate load.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-05-R03 — Specify constant load.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-05-R04 — Specify register copy.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-05-R05 — Specify clear.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-05-R01 | Specify MOV | NOT RUN | — |
| BR-430-05-R02 | Specify immediate load | NOT RUN | — |
| BR-430-05-R03 | Specify constant load | NOT RUN | — |
| BR-430-05-R04 | Specify register copy | NOT RUN | — |
| BR-430-05-R05 | Specify clear | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
