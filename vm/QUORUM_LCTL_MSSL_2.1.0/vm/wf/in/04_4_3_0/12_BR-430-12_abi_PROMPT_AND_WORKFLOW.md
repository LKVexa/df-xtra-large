...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **ABI** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.h::BR_STACK_WORDS`
- `src/brvm.c::br_vm_init`

### Atomic requirements
- **BR-430-12-R01:** Define argument registers
- **BR-430-12-R02:** Define return registers
- **BR-430-12-R03:** Define scratch registers
- **BR-430-12-R04:** Define preserved registers
- **BR-430-12-R05:** Define stack convention
- **BR-430-12-R06:** Define service calling convention
- **BR-430-12-R07:** Define trap frame
- **BR-430-12-R08:** Define capability passing
- **BR-430-12-R09:** Define ABI version number
...
Define the invariants for **ABI** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-12-R01 — Define argument registers.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-12-R02 — Define return registers.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-12-R03 — Define scratch registers.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-12-R04 — Define preserved registers.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-12-R05 — Define stack convention.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-12-R06 — Define service calling convention.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-12-R07 — Define trap frame.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-430-12-R08 — Define capability passing.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
9. **BR-430-12-R09 — Define ABI version number.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-12-R01 | Define argument registers | NOT RUN | — |
| BR-430-12-R02 | Define return registers | NOT RUN | — |
| BR-430-12-R03 | Define scratch registers | NOT RUN | — |
| BR-430-12-R04 | Define preserved registers | NOT RUN | — |
| BR-430-12-R05 | Define stack convention | NOT RUN | — |
| BR-430-12-R06 | Define service calling convention | NOT RUN | — |
| BR-430-12-R07 | Define trap frame | NOT RUN | — |
| BR-430-12-R08 | Define capability passing | NOT RUN | — |
| BR-430-12-R09 | Define ABI version number | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
