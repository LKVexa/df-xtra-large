...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Service ABI** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-13-R01:** Define console service
- **BR-430-13-R02:** Define storage service
- **BR-430-13-R03:** Define entropy service
- **BR-430-13-R04:** Define monotonic-counter service
- **BR-430-13-R05:** Define timer service
- **BR-430-13-R06:** Define diagnostic service
- **BR-430-13-R07:** Define mailbox service
- **BR-430-13-R08:** Define device enumeration service
...
Define the invariants for **Service ABI** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-13-R01 — Define console service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-13-R02 — Define storage service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-13-R03 — Define entropy service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-13-R04 — Define monotonic-counter service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-13-R05 — Define timer service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-13-R06 — Define diagnostic service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-13-R07 — Define mailbox service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-430-13-R08 — Define device enumeration service.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-13_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-13_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-13_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-13_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-13-R01 | Define console service | NOT RUN | — |
| BR-430-13-R02 | Define storage service | NOT RUN | — |
| BR-430-13-R03 | Define entropy service | NOT RUN | — |
| BR-430-13-R04 | Define monotonic-counter service | NOT RUN | — |
| BR-430-13-R05 | Define timer service | NOT RUN | — |
| BR-430-13-R06 | Define diagnostic service | NOT RUN | — |
| BR-430-13-R07 | Define mailbox service | NOT RUN | — |
| BR-430-13-R08 | Define device enumeration service | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-13**. The next package may consume this package only when its declared prerequisite status is satisfied.
