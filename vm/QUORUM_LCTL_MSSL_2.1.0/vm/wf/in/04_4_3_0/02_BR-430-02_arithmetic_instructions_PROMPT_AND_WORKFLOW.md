# QUORUM BR-430-02 — Arithmetic instructions
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Arithmetic instructions** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-02-R01:** Specify ADD
- **BR-430-02-R02:** Specify SUB
- **BR-430-02-R03:** Specify MUL
- **BR-430-02-R04:** Specify DIV
- **BR-430-02-R05:** Specify MOD
- **BR-430-02-R06:** Specify NEG if required
- **BR-430-02-R07:** Specify compare
- **BR-430-02-R08:** Specify min/max only if justified
- **BR-430-02-R09:** Specify carry behavior
- **BR-430-02-R10:** Specify overflow semantics
- **BR-430-02-R11:** Specify divide-by-zero trap
...
Define the invariants for **Arithmetic instructions** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-02-R01 — Specify ADD.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-02-R02 — Specify SUB.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-02-R03 — Specify MUL.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-02-R04 — Specify DIV.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-02-R05 — Specify MOD.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-02-R06 — Specify NEG if required.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-02-R07 — Specify compare.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-430-02-R08 — Specify min/max only if justified.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
9. **BR-430-02-R09 — Specify carry behavior.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
10. **BR-430-02-R10 — Specify overflow semantics.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
11. **BR-430-02-R11 — Specify divide-by-zero trap.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-02-R01 | Specify ADD | NOT RUN | — |
| BR-430-02-R02 | Specify SUB | NOT RUN | — |
| BR-430-02-R03 | Specify MUL | NOT RUN | — |
| BR-430-02-R04 | Specify DIV | NOT RUN | — |
| BR-430-02-R05 | Specify MOD | NOT RUN | — |
| BR-430-02-R06 | Specify NEG if required | NOT RUN | — |
| BR-430-02-R07 | Specify compare | NOT RUN | — |
| BR-430-02-R08 | Specify min/max only if justified | NOT RUN | — |
| BR-430-02-R09 | Specify carry behavior | NOT RUN | — |
| BR-430-02-R10 | Specify overflow semantics | NOT RUN | — |
| BR-430-02-R11 | Specify divide-by-zero trap | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
