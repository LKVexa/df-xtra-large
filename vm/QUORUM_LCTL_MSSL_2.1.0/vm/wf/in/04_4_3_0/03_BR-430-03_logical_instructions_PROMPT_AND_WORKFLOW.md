# QUORUM BR-430-03 — Logical instructions
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Logical instructions** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-03-R01:** Specify AND
- **BR-430-03-R02:** Specify OR
- **BR-430-03-R03:** Specify XOR
- **BR-430-03-R04:** Specify NOT
- **BR-430-03-R05:** Specify bit-test
- **BR-430-03-R06:** Specify masks
- **BR-430-03-R07:** Add population/count only if justified
...
Define the invariants for **Logical instructions** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-03-R01 — Specify AND.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-03-R02 — Specify OR.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-03-R03 — Specify XOR.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-03-R04 — Specify NOT.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-03-R05 — Specify bit-test.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-03-R06 — Specify masks.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-430-03-R07 — Add population/count only if justified.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-430-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-03-R01 | Specify AND | NOT RUN | — |
| BR-430-03-R02 | Specify OR | NOT RUN | — |
| BR-430-03-R03 | Specify XOR | NOT RUN | — |
| BR-430-03-R04 | Specify NOT | NOT RUN | — |
| BR-430-03-R05 | Specify bit-test | NOT RUN | — |
| BR-430-03-R06 | Specify masks | NOT RUN | — |
| BR-430-03-R07 | Add population/count only if justified | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
