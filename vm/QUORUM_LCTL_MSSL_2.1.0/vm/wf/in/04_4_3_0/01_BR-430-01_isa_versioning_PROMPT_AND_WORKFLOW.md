# QUORUM BR-430-01 — ISA versioning
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **ISA versioning** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-430-01-R01:** Define major ISA version
- **BR-430-01-R02:** Define minor ISA version
- **BR-430-01-R03:** Define compatibility rules
- **BR-430-01-R04:** Define deprecated opcode rules
- **BR-430-01-R05:** Define extension mechanism
- **BR-430-01-R06:** Define feature flags
...
Define the invariants for **ISA versioning** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-430-01-R01 — Define major ISA version.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-430-01-R02 — Define minor ISA version.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-430-01-R03 — Define compatibility rules.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-430-01-R04 — Define deprecated opcode rules.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-430-01-R05 — Define extension mechanism.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-430-01-R06 — Define feature flags.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
...
### Q7 — Evidence package
Generate:
- `BR-430-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-430-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-430-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-430-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-430-01-R01 | Define major ISA version | NOT RUN | — |
| BR-430-01-R02 | Define minor ISA version | NOT RUN | — |
| BR-430-01-R03 | Define compatibility rules | NOT RUN | — |
| BR-430-01-R04 | Define deprecated opcode rules | NOT RUN | — |
| BR-430-01-R05 | Define extension mechanism | NOT RUN | — |
| BR-430-01-R06 | Define feature flags | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-430-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
