# QUORUM BR-480-09 — Capability attacks
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Capability attacks** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-09-R01:** Test undeclared device
- **BR-480-09-R02:** Test privilege escalation
- **BR-480-09-R03:** Test forged capability
- **BR-480-09-R04:** Test stale capability
- **BR-480-09-R05:** Test capability reuse across VM instances
...
Define the invariants for **Capability attacks** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-09-R01 — Test undeclared device.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-09-R02 — Test privilege escalation.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-480-09-R03 — Test forged capability.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-480-09-R04 — Test stale capability.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-480-09-R05 — Test capability reuse across VM instances.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-480-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-09-R01 | Test undeclared device | NOT RUN | — |
| BR-480-09-R02 | Test privilege escalation | NOT RUN | — |
| BR-480-09-R03 | Test forged capability | NOT RUN | — |
| BR-480-09-R04 | Test stale capability | NOT RUN | — |
| BR-480-09-R05 | Test capability reuse across VM instances | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
