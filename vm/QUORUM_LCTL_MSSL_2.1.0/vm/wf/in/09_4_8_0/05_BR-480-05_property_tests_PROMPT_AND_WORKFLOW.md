# QUORUM BR-480-05 — Property tests
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Property tests** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-05-R01:** Verify a + 0 = a
- **BR-480-05-R02:** Verify a XOR a = 0
- **BR-480-05-R03:** Verify modular invariants
- **BR-480-05-R04:** Verify load/store roundtrip
- **BR-480-05-R05:** Verify push/pop invariants
- **BR-480-05-R06:** Verify deterministic execution invariants
...
Define the invariants for **Property tests** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-05-R01 — Verify a + 0 = a.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-480-05-R02 — Verify a XOR a = 0.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-05-R03 — Verify modular invariants.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-05-R04 — Verify load/store roundtrip.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-05-R05 — Verify push/pop invariants.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-05-R06 — Verify deterministic execution invariants.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-05-R01 | Verify a + 0 = a | NOT RUN | — |
| BR-480-05-R02 | Verify a XOR a = 0 | NOT RUN | — |
| BR-480-05-R03 | Verify modular invariants | NOT RUN | — |
| BR-480-05-R04 | Verify load/store roundtrip | NOT RUN | — |
| BR-480-05-R05 | Verify push/pop invariants | NOT RUN | — |
| BR-480-05-R06 | Verify deterministic execution invariants | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
