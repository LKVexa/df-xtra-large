# QUORUM BR-480-16 — Security review
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Security review** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-16-R01:** Create threat model
- **BR-480-16-R02:** Inventory attack surface
- **BR-480-16-R03:** Document trust boundaries
- **BR-480-16-R04:** Review cryptography
- **BR-480-16-R05:** Review parsers
- **BR-480-16-R06:** Review memory safety
- **BR-480-16-R07:** Review capability model
- **BR-480-16-R08:** Review persistence
...
Define the invariants for **Security review** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-16-R01 — Create threat model.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-480-16-R02 — Inventory attack surface.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-16-R03 — Document trust boundaries.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-480-16-R04 — Review cryptography.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-16-R05 — Review parsers.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-16-R06 — Review memory safety.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-480-16-R07 — Review capability model.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-480-16-R08 — Review persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-16_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-16_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-16_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-16_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-16-R01 | Create threat model | NOT RUN | — |
| BR-480-16-R02 | Inventory attack surface | NOT RUN | — |
| BR-480-16-R03 | Document trust boundaries | NOT RUN | — |
| BR-480-16-R04 | Review cryptography | NOT RUN | — |
| BR-480-16-R05 | Review parsers | NOT RUN | — |
| BR-480-16-R06 | Review memory safety | NOT RUN | — |
| BR-480-16-R07 | Review capability model | NOT RUN | — |
| BR-480-16-R08 | Review persistence | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-16**. The next package may consume this package only when its declared prerequisite status is satisfied.
