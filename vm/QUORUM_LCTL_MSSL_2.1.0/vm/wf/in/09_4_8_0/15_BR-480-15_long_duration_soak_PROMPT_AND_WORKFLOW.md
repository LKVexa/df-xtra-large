# QUORUM BR-480-15 — Long-duration soak
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Long-duration soak** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-15-R01:** Run at least 72 hours
- **BR-480-15-R02:** Repeat execution
- **BR-480-15-R03:** Repeat reset
- **BR-480-15-R04:** Repeat load/unload
- **BR-480-15-R05:** Repeat persistence
- **BR-480-15-R06:** Repeat rollback attempts
- **BR-480-15-R07:** Monitor memory leaks
- **BR-480-15-R08:** Monitor resource ceilings
...
Define the invariants for **Long-duration soak** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-15-R01 — Run at least 72 hours.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-480-15-R02 — Repeat execution.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-15-R03 — Repeat reset.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-15-R04 — Repeat load/unload.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-15-R05 — Repeat persistence.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-15-R06 — Repeat rollback attempts.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-480-15-R07 — Monitor memory leaks.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-480-15-R08 — Monitor resource ceilings.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
...
### Q7 — Evidence package
Generate:
- `BR-480-15_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-15_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-15_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-15_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-15-R01 | Run at least 72 hours | NOT RUN | — |
| BR-480-15-R02 | Repeat execution | NOT RUN | — |
| BR-480-15-R03 | Repeat reset | NOT RUN | — |
| BR-480-15-R04 | Repeat load/unload | NOT RUN | — |
| BR-480-15-R05 | Repeat persistence | NOT RUN | — |
| BR-480-15-R06 | Repeat rollback attempts | NOT RUN | — |
| BR-480-15-R07 | Monitor memory leaks | NOT RUN | — |
| BR-480-15-R08 | Monitor resource ceilings | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-15**. The next package may consume this package only when its declared prerequisite status is satisfied.
