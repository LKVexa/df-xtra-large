# QUORUM BR-480-12 — Persistence attacks
...
### Atomic requirements
- **BR-480-12-R01:** Corrupt slot A
- **BR-480-12-R02:** Corrupt slot B
- **BR-480-12-R03:** Corrupt both slots
- **BR-480-12-R04:** Corrupt control record
- **BR-480-12-R05:** Mismatch slot/control
- **BR-480-12-R06:** Interrupt write
- **BR-480-12-R07:** Replay stale filesystem copy
...
Define the invariants for **Persistence attacks** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-12-R01 — Corrupt slot A.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-12-R02 — Corrupt slot B.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-480-12-R03 — Corrupt both slots.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-480-12-R04 — Corrupt control record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
5. **BR-480-12-R05 — Mismatch slot/control.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-12-R06 — Interrupt write.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-480-12-R07 — Replay stale filesystem copy.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-12-R01 | Corrupt slot A | NOT RUN | — |
| BR-480-12-R02 | Corrupt slot B | NOT RUN | — |
| BR-480-12-R03 | Corrupt both slots | NOT RUN | — |
| BR-480-12-R04 | Corrupt control record | NOT RUN | — |
| BR-480-12-R05 | Mismatch slot/control | NOT RUN | — |
| BR-480-12-R06 | Interrupt write | NOT RUN | — |
| BR-480-12-R07 | Replay stale filesystem copy | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
