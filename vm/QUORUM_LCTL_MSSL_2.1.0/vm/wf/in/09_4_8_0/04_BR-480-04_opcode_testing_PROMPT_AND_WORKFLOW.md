# QUORUM BR-480-04 — Opcode testing
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Opcode testing** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-04-R01:** Test every opcode
- **BR-480-04-R02:** Test every mode
- **BR-480-04-R03:** Test every operand position
- **BR-480-04-R04:** Test every trap
- **BR-480-04-R05:** Test boundary register indexes
- **BR-480-04-R06:** Test boundary memory offsets
...
Define the invariants for **Opcode testing** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-04-R01 — Test every opcode.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-04-R02 — Test every mode.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-480-04-R03 — Test every operand position.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-480-04-R04 — Test every trap.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-480-04-R05 — Test boundary register indexes.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
6. **BR-480-04-R06 — Test boundary memory offsets.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-480-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-04-R01 | Test every opcode | NOT RUN | — |
| BR-480-04-R02 | Test every mode | NOT RUN | — |
| BR-480-04-R03 | Test every operand position | NOT RUN | — |
| BR-480-04-R04 | Test every trap | NOT RUN | — |
| BR-480-04-R05 | Test boundary register indexes | NOT RUN | — |
| BR-480-04-R06 | Test boundary memory offsets | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
