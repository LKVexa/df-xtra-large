# QUORUM BR-480-02 — Independent verifier
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Independent verifier** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-02-R01:** Validate header
- **BR-480-02-R02:** Validate ISA
- **BR-480-02-R03:** Validate ABI
- **BR-480-02-R04:** Validate instructions
- **BR-480-02-R05:** Validate control flow
- **BR-480-02-R06:** Validate capabilities
- **BR-480-02-R07:** Validate hashes
- **BR-480-02-R08:** Validate signature metadata
...
Define the invariants for **Independent verifier** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-02-R01 — Validate header.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-480-02-R02 — Validate ISA.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-02-R03 — Validate ABI.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-02-R04 — Validate instructions.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-02-R05 — Validate control flow.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-02-R06 — Validate capabilities.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-480-02-R07 — Validate hashes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
8. **BR-480-02-R08 — Validate signature metadata.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-02-R01 | Validate header | NOT RUN | — |
| BR-480-02-R02 | Validate ISA | NOT RUN | — |
| BR-480-02-R03 | Validate ABI | NOT RUN | — |
| BR-480-02-R04 | Validate instructions | NOT RUN | — |
| BR-480-02-R05 | Validate control flow | NOT RUN | — |
| BR-480-02-R06 | Validate capabilities | NOT RUN | — |
| BR-480-02-R07 | Validate hashes | NOT RUN | — |
| BR-480-02-R08 | Validate signature metadata | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
