# QUORUM BR-480-14 — Reproducibility
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Reproducibility** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-14-R01:** Use independent build machine
- **BR-480-14-R02:** Use independent operator
- **BR-480-14-R03:** Require identical BRIM
- **BR-480-14-R04:** Require identical hashes
- **BR-480-14-R05:** Require identical conformance result
...
Define the invariants for **Reproducibility** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-14-R01 — Use independent build machine.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-480-14-R02 — Use independent operator.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-480-14-R03 — Require identical BRIM.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
4. **BR-480-14-R04 — Require identical hashes.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
5. **BR-480-14-R05 — Require identical conformance result.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-480-14_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-14_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-14_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-14_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-14-R01 | Use independent build machine | NOT RUN | — |
| BR-480-14-R02 | Use independent operator | NOT RUN | — |
| BR-480-14-R03 | Require identical BRIM | NOT RUN | — |
| BR-480-14-R04 | Require identical hashes | NOT RUN | — |
| BR-480-14-R05 | Require identical conformance result | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-14**. The next package may consume this package only when its declared prerequisite status is satisfied.
