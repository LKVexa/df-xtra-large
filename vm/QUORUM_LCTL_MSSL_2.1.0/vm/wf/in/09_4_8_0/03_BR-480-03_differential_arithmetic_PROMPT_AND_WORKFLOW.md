# QUORUM BR-480-03 — Differential arithmetic
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Differential arithmetic** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-03-R01:** Use independent arbitrary-precision oracle
- **BR-480-03-R02:** Generate random vectors
- **BR-480-03-R03:** Generate boundary vectors
- **BR-480-03-R04:** Generate signed vectors
- **BR-480-03-R05:** Generate unsigned vectors
- **BR-480-03-R06:** Generate modular vectors
- **BR-480-03-R07:** Generate maximum-width vectors
...
Define the invariants for **Differential arithmetic** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-03-R01 — Use independent arbitrary-precision oracle.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-480-03-R02 — Generate random vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-03-R03 — Generate boundary vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-03-R04 — Generate signed vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-03-R05 — Generate unsigned vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-03-R06 — Generate modular vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
7. **BR-480-03-R07 — Generate maximum-width vectors.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-03-R01 | Use independent arbitrary-precision oracle | NOT RUN | — |
| BR-480-03-R02 | Generate random vectors | NOT RUN | — |
| BR-480-03-R03 | Generate boundary vectors | NOT RUN | — |
| BR-480-03-R04 | Generate signed vectors | NOT RUN | — |
| BR-480-03-R05 | Generate unsigned vectors | NOT RUN | — |
| BR-480-03-R06 | Generate modular vectors | NOT RUN | — |
| BR-480-03-R07 | Generate maximum-width vectors | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
