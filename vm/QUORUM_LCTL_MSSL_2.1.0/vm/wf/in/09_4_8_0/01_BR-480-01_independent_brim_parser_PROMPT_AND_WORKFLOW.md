# QUORUM BR-480-01 — Independent BRIM parser
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Independent BRIM parser** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-01-R01:** Create separate implementation
- **BR-480-01-R02:** Use independent codebase where practical
- **BR-480-01-R03:** Compare parser results
- **BR-480-01-R04:** Detect implementation-specific assumptions
...
Define the invariants for **Independent BRIM parser** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-01-R01 — Create separate implementation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-480-01-R02 — Use independent codebase where practical.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-480-01-R03 — Compare parser results.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-01-R04 — Detect implementation-specific assumptions.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-480-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-01-R01 | Create separate implementation | NOT RUN | — |
| BR-480-01-R02 | Use independent codebase where practical | NOT RUN | — |
| BR-480-01-R03 | Compare parser results | NOT RUN | — |
| BR-480-01-R04 | Detect implementation-specific assumptions | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
