# QUORUM BR-480-08 — Control-flow attacks
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Control-flow attacks** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-08-R01:** Test branch out of bounds
- **BR-480-08-R02:** Test branch into data
- **BR-480-08-R03:** Test invalid return
- **BR-480-08-R04:** Test stack corruption
- **BR-480-08-R05:** Test malformed indirect target
- **BR-480-08-R06:** Test unreachable privileged instruction
...
Define the invariants for **Control-flow attacks** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-08-R01 — Test branch out of bounds.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-08-R02 — Test branch into data.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-480-08-R03 — Test invalid return.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-480-08-R04 — Test stack corruption.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-480-08-R05 — Test malformed indirect target.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
6. **BR-480-08-R06 — Test unreachable privileged instruction.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-480-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-08-R01 | Test branch out of bounds | NOT RUN | — |
| BR-480-08-R02 | Test branch into data | NOT RUN | — |
| BR-480-08-R03 | Test invalid return | NOT RUN | — |
| BR-480-08-R04 | Test stack corruption | NOT RUN | — |
| BR-480-08-R05 | Test malformed indirect target | NOT RUN | — |
| BR-480-08-R06 | Test unreachable privileged instruction | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
