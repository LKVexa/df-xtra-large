# QUORUM BR-480-11 — Rollback attacks
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Rollback attacks** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-11-R01:** Try older valid image
- **BR-480-11-R02:** Try older control record
- **BR-480-11-R03:** Mix old/new slots
- **BR-480-11-R04:** Replay signed update
- **BR-480-11-R05:** Test generation wrap conditions
- **BR-480-11-R06:** Test issuer epoch rollback
...
Define the invariants for **Rollback attacks** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-11-R01 — Try older valid image.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-11-R02 — Try older control record.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
3. **BR-480-11-R03 — Mix old/new slots.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-11-R04 — Replay signed update.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-11-R05 — Test generation wrap conditions.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
6. **BR-480-11-R06 — Test issuer epoch rollback.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-480-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-11-R01 | Try older valid image | NOT RUN | — |
| BR-480-11-R02 | Try older control record | NOT RUN | — |
| BR-480-11-R03 | Mix old/new slots | NOT RUN | — |
| BR-480-11-R04 | Replay signed update | NOT RUN | — |
| BR-480-11-R05 | Test generation wrap conditions | NOT RUN | — |
| BR-480-11-R06 | Test issuer epoch rollback | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
