# QUORUM BR-480-07 — Memory safety
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Memory safety** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-07-R01:** Run ASan
- **BR-480-07-R02:** Run UBSan
- **BR-480-07-R03:** Run MSan where practical
- **BR-480-07-R04:** Run valgrind-style checks where applicable
- **BR-480-07-R05:** Run static analysis
- **BR-480-07-R06:** Enable bounds assertions
...
Define the invariants for **Memory safety** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-07-R01 — Run ASan.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-480-07-R02 — Run UBSan.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-480-07-R03 — Run MSan where practical.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-480-07-R04 — Run valgrind-style checks where applicable.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-480-07-R05 — Run static analysis.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-480-07-R06 — Enable bounds assertions.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-480-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-07-R01 | Run ASan | NOT RUN | — |
| BR-480-07-R02 | Run UBSan | NOT RUN | — |
| BR-480-07-R03 | Run MSan where practical | NOT RUN | — |
| BR-480-07-R04 | Run valgrind-style checks where applicable | NOT RUN | — |
| BR-480-07-R05 | Run static analysis | NOT RUN | — |
| BR-480-07-R06 | Enable bounds assertions | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
