...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Fuzzing** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-480-06-R01:** Fuzz LCTL parser
- **BR-480-06-R02:** Fuzz BRIR parser
- **BR-480-06-R03:** Fuzz BRIM parser
- **BR-480-06-R04:** Fuzz image loader
- **BR-480-06-R05:** Fuzz APDU parser
- **BR-480-06-R06:** Fuzz service ABI
- **BR-480-06-R07:** Fuzz persistence records
- **BR-480-06-R08:** Fuzz malformed signatures
- **BR-480-06-R09:** Fuzz corrupted images
...
Define the invariants for **Fuzzing** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-480-06-R01 — Fuzz LCTL parser.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
2. **BR-480-06-R02 — Fuzz BRIR parser.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
3. **BR-480-06-R03 — Fuzz BRIM parser.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-480-06-R04 — Fuzz image loader.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
5. **BR-480-06-R05 — Fuzz APDU parser.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
6. **BR-480-06-R06 — Fuzz service ABI.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
7. **BR-480-06-R07 — Fuzz persistence records.** Instrument the implementation, collect the value from execution rather than assertion, and emit reproducible evidence with units and provenance.
8. **BR-480-06-R08 — Fuzz malformed signatures.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
9. **BR-480-06-R09 — Fuzz corrupted images.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-480-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-480-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-480-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-480-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-480-06-R01 | Fuzz LCTL parser | NOT RUN | — |
| BR-480-06-R02 | Fuzz BRIR parser | NOT RUN | — |
| BR-480-06-R03 | Fuzz BRIM parser | NOT RUN | — |
| BR-480-06-R04 | Fuzz image loader | NOT RUN | — |
| BR-480-06-R05 | Fuzz APDU parser | NOT RUN | — |
| BR-480-06-R06 | Fuzz service ABI | NOT RUN | — |
| BR-480-06-R07 | Fuzz persistence records | NOT RUN | — |
| BR-480-06-R08 | Fuzz malformed signatures | NOT RUN | — |
| BR-480-06-R09 | Fuzz corrupted images | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-480-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
