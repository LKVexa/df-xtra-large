...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Determinism** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-11-R01:** Allocation strategy must not alter observable semantics
- **BR-460-11-R02:** Keep arithmetic results stable
- **BR-460-11-R03:** Keep trap behavior stable
- **BR-460-11-R04:** Keep instruction counts stable
...
Define the invariants for **Determinism** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-11-R01 — Allocation strategy must not alter observable semantics.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-460-11-R02 — Keep arithmetic results stable.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-460-11-R03 — Keep trap behavior stable.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-11-R04 — Keep instruction counts stable.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-11-R01 | Allocation strategy must not alter observable semantics | NOT RUN | — |
| BR-460-11-R02 | Keep arithmetic results stable | NOT RUN | — |
| BR-460-11-R03 | Keep trap behavior stable | NOT RUN | — |
| BR-460-11-R04 | Keep instruction counts stable | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
