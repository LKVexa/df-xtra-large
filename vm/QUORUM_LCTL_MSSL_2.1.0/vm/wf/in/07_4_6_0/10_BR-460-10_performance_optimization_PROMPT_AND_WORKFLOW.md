# QUORUM BR-460-10 — Performance optimization
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Performance optimization** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-10-R01:** Implement small-integer fast path
- **BR-460-10-R02:** Implement zero fast path
- **BR-460-10-R03:** Implement one fast path
- **BR-460-10-R04:** Implement sparse high-bit fast path
- **BR-460-10-R05:** Use vectorized limbs only where portable
- **BR-460-10-R06:** Perform constant folding in toolchain
...
Define the invariants for **Performance optimization** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-10-R01 — Implement small-integer fast path.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-10-R02 — Implement zero fast path.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-460-10-R03 — Implement one fast path.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-460-10-R04 — Implement sparse high-bit fast path.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-460-10-R05 — Use vectorized limbs only where portable.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-460-10-R06 — Perform constant folding in toolchain.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-10-R01 | Implement small-integer fast path | NOT RUN | — |
| BR-460-10-R02 | Implement zero fast path | NOT RUN | — |
| BR-460-10-R03 | Implement one fast path | NOT RUN | — |
| BR-460-10-R04 | Implement sparse high-bit fast path | NOT RUN | — |
| BR-460-10-R05 | Use vectorized limbs only where portable | NOT RUN | — |
| BR-460-10-R06 | Perform constant folding in toolchain | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
