# QUORUM BR-460-05 — Constant pool
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Constant pool** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-05-R01:** Deduplicate constants
- **BR-460-05-R02:** Make constants immutable
- **BR-460-05-R03:** Share zero/one values
- **BR-460-05-R04:** Precompute masks only where economical
...
Define the invariants for **Constant pool** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-05-R01 — Deduplicate constants.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-460-05-R02 — Make constants immutable.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-460-05-R03 — Share zero/one values.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-05-R04 — Precompute masks only where economical.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-05_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-05_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-05_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-05_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-05-R01 | Deduplicate constants | NOT RUN | — |
| BR-460-05-R02 | Make constants immutable | NOT RUN | — |
| BR-460-05-R03 | Share zero/one values | NOT RUN | — |
| BR-460-05-R04 | Precompute masks only where economical | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-05**. The next package may consume this package only when its declared prerequisite status is satisfied.
