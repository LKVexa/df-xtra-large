# QUORUM BR-460-06 — Scratch allocator
...
- `src/brvm.c::word_*`
- `src/brvm.h::br_vm.tmp/tmp2`
- `src/brvm.c::br_vm_init`

### Atomic requirements
- **BR-460-06-R01:** Reuse scratch words
- **BR-460-06-R02:** Use bounded arena
- **BR-460-06-R03:** Allocate deterministically
- **BR-460-06-R04:** Prevent arbitrary fragmentation
- **BR-460-06-R05:** Fail safely on exhaustion
...
Define the invariants for **Scratch allocator** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-06-R01 — Reuse scratch words.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-06-R02 — Use bounded arena.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-460-06-R03 — Allocate deterministically.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-06-R04 — Prevent arbitrary fragmentation.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
5. **BR-460-06-R05 — Fail safely on exhaustion.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-06-R01 | Reuse scratch words | NOT RUN | — |
| BR-460-06-R02 | Use bounded arena | NOT RUN | — |
| BR-460-06-R03 | Allocate deterministically | NOT RUN | — |
| BR-460-06-R04 | Prevent arbitrary fragmentation | NOT RUN | — |
| BR-460-06-R05 | Fail safely on exhaustion | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
