# QUORUM BR-460-04 — Stack redesign
...
### Atomic requirements
- **BR-460-04-R01:** Make stack contain word handles/descriptors
- **BR-460-04-R02:** Do not preallocate 256 complete megabit words
- **BR-460-04-R03:** Allocate values as pushed
- **BR-460-04-R04:** Reclaim on pop
- **BR-460-04-R05:** Enforce byte quota and depth quota
...
Define the invariants for **Stack redesign** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-04-R01 — Make stack contain word handles/descriptors.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-04-R02 — Do not preallocate 256 complete megabit words.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-460-04-R03 — Allocate values as pushed.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-04-R04 — Reclaim on pop.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-460-04-R05 — Enforce byte quota and depth quota.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-04_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-04_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-04_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-04_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-04-R01 | Make stack contain word handles/descriptors | NOT RUN | — |
| BR-460-04-R02 | Do not preallocate 256 complete megabit words | NOT RUN | — |
| BR-460-04-R03 | Allocate values as pushed | NOT RUN | — |
| BR-460-04-R04 | Reclaim on pop | NOT RUN | — |
| BR-460-04-R05 | Enforce byte quota and depth quota | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-04**. The next package may consume this package only when its declared prerequisite status is satisfied.
