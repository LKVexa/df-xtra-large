# QUORUM BR-460-08 — Memory quotas
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Memory quotas** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-08-R01:** Set per-VM byte ceiling
- **BR-460-08-R02:** Set per-register maximum
- **BR-460-08-R03:** Set stack ceiling
- **BR-460-08-R04:** Set scratch ceiling
- **BR-460-08-R05:** Set image ceiling
- **BR-460-08-R06:** Set device-buffer ceiling
...
Define the invariants for **Memory quotas** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-08-R01 — Set per-VM byte ceiling.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-460-08-R02 — Set per-register maximum.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-460-08-R03 — Set stack ceiling.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-08-R04 — Set scratch ceiling.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-460-08-R05 — Set image ceiling.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-460-08-R06 — Set device-buffer ceiling.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-08-R01 | Set per-VM byte ceiling | NOT RUN | — |
| BR-460-08-R02 | Set per-register maximum | NOT RUN | — |
| BR-460-08-R03 | Set stack ceiling | NOT RUN | — |
| BR-460-08-R04 | Set scratch ceiling | NOT RUN | — |
| BR-460-08-R05 | Set image ceiling | NOT RUN | — |
| BR-460-08-R06 | Set device-buffer ceiling | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
