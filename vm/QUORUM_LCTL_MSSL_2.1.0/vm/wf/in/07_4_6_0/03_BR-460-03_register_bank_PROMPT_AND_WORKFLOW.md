# QUORUM BR-460-03 — Register bank
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Register bank** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-03-R01:** Make registers hold word descriptors
- **BR-460-03-R02:** Define reference-count or ownership model
- **BR-460-03-R03:** Use copy-on-write
- **BR-460-03-R04:** Zero-initialize without physical allocation
- **BR-460-03-R05:** Destroy deterministically
...
Define the invariants for **Register bank** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-03-R01 — Make registers hold word descriptors.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-03-R02 — Define reference-count or ownership model.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-460-03-R03 — Use copy-on-write.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-460-03-R04 — Zero-initialize without physical allocation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-460-03-R05 — Destroy deterministically.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-03-R01 | Make registers hold word descriptors | NOT RUN | — |
| BR-460-03-R02 | Define reference-count or ownership model | NOT RUN | — |
| BR-460-03-R03 | Use copy-on-write | NOT RUN | — |
| BR-460-03-R04 | Zero-initialize without physical allocation | NOT RUN | — |
| BR-460-03-R05 | Destroy deterministically | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
