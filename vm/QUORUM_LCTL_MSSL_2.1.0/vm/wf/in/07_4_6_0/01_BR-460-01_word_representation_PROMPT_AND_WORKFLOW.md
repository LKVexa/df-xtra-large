# QUORUM BR-460-01 — Word representation

**Release sequence:** BOTTLE ROCKET 4.6.0 — Wide-State Memory & Performance Architecture  
**Purpose:** Preserve 1,048,576-bit capability without requiring tens of megabytes for mostly empty values.  
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Word representation** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.c::word_*`
- `src/brvm.h::br_vm`
- `src/brvm.c::regp/active_limbs`
- `src/brvm.c::br_vm_init`
- `src/brvm.c`
- `host/brctl.c::assemble`

### Atomic requirements
- **BR-460-01-R01:** Support dense representation
- **BR-460-01-R02:** Support sparse representation
- **BR-460-01-R03:** Support small-value representation
- **BR-460-01-R04:** Use zero singleton
- **BR-460-01-R05:** Support copy-on-write representation
- **BR-460-01-R06:** Support immutable constant representation
...
Define the invariants for **Word representation** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-01-R01 — Support dense representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-01-R02 — Support sparse representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-460-01-R03 — Support small-value representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-460-01-R04 — Use zero singleton.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-460-01-R05 — Support copy-on-write representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-460-01-R06 — Support immutable constant representation.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-460-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-01-R01 | Support dense representation | NOT RUN | — |
| BR-460-01-R02 | Support sparse representation | NOT RUN | — |
| BR-460-01-R03 | Support small-value representation | NOT RUN | — |
| BR-460-01-R04 | Use zero singleton | NOT RUN | — |
| BR-460-01-R05 | Support copy-on-write representation | NOT RUN | — |
| BR-460-01-R06 | Support immutable constant representation | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
