# QUORUM BR-460-12 — Acceptance gate
...
### Atomic requirements
- **BR-460-12-R01:** Simple workloads no longer require roughly 36 MB architectural allocation
- **BR-460-12-R02:** Maximum 1,048,576-bit words remain supported
- **BR-460-12-R03:** Memory limits enforced
- **BR-460-12-R04:** Performance improves without semantic changes
...
### Q2 — Implementation
1. **BR-460-12-R01 — Simple workloads no longer require roughly 36 MB architectural allocation.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
2. **BR-460-12-R02 — Maximum 1,048,576-bit words remain supported.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-460-12-R03 — Memory limits enforced.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-12-R04 — Performance improves without semantic changes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-460-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-12-R01 | Simple workloads no longer require roughly 36 MB architectural allocation | NOT RUN | — |
| BR-460-12-R02 | Maximum 1,048,576-bit words remain supported | NOT RUN | — |
| BR-460-12-R03 | Memory limits enforced | NOT RUN | — |
| BR-460-12-R04 | Performance improves without semantic changes | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
