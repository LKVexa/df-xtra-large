# QUORUM BR-460-07 — Arithmetic algorithms
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Arithmetic algorithms** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-460-07-R01:** Use limb-based add/sub
- **BR-460-07-R02:** Choose multiplication strategy by operand size
- **BR-460-07-R03:** Choose division strategy by operand size
- **BR-460-07-R04:** Implement modular algorithms
- **BR-460-07-R05:** Normalize after operation
- **BR-460-07-R06:** Avoid processing unused upper limbs
...
Define the invariants for **Arithmetic algorithms** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-460-07-R01 — Use limb-based add/sub.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-460-07-R02 — Choose multiplication strategy by operand size.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-460-07-R03 — Choose division strategy by operand size.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-460-07-R04 — Implement modular algorithms.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-460-07-R05 — Normalize after operation.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-460-07-R06 — Avoid processing unused upper limbs.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-460-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-460-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-460-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-460-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-460-07-R01 | Use limb-based add/sub | NOT RUN | — |
| BR-460-07-R02 | Choose multiplication strategy by operand size | NOT RUN | — |
| BR-460-07-R03 | Choose division strategy by operand size | NOT RUN | — |
| BR-460-07-R04 | Implement modular algorithms | NOT RUN | — |
| BR-460-07-R05 | Normalize after operation | NOT RUN | — |
| BR-460-07-R06 | Avoid processing unused upper limbs | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-460-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
