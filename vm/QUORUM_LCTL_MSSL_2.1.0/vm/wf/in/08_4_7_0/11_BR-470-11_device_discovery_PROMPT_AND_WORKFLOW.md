# QUORUM BR-470-11 — Device discovery
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Device discovery** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-11-R01:** Enumerate available devices
- **BR-470-11-R02:** Expose versions
- **BR-470-11-R03:** Expose capabilities
- **BR-470-11-R04:** Expose limits
- **BR-470-11-R05:** Use deterministic ordering
...
Define the invariants for **Device discovery** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-11-R01 — Enumerate available devices.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-11-R02 — Expose versions.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-470-11-R03 — Expose capabilities.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-11-R04 — Expose limits.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-11-R05 — Use deterministic ordering.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
...
### Q7 — Evidence package
Generate:
- `BR-470-11_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-11_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-11_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-11_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-11-R01 | Enumerate available devices | NOT RUN | — |
| BR-470-11-R02 | Expose versions | NOT RUN | — |
| BR-470-11-R03 | Expose capabilities | NOT RUN | — |
| BR-470-11-R04 | Expose limits | NOT RUN | — |
| BR-470-11-R05 | Use deterministic ordering | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-11**. The next package may consume this package only when its declared prerequisite status is satisfied.
