# QUORUM BR-470-09 — Diagnostic device
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Diagnostic device** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-09-R01:** Expose VM state summary
- **BR-470-09-R02:** Expose trap state
- **BR-470-09-R03:** Expose resource usage
- **BR-470-09-R04:** Expose image identity
- **BR-470-09-R05:** Expose build identity
- **BR-470-09-R06:** Prevent secret leakage
...
Define the invariants for **Diagnostic device** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-09-R01 — Expose VM state summary.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-09-R02 — Expose trap state.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-470-09-R03 — Expose resource usage.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-09-R04 — Expose image identity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-09-R05 — Expose build identity.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-470-09-R06 — Prevent secret leakage.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-470-09_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-09_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-09_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-09_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-09-R01 | Expose VM state summary | NOT RUN | — |
| BR-470-09-R02 | Expose trap state | NOT RUN | — |
| BR-470-09-R03 | Expose resource usage | NOT RUN | — |
| BR-470-09-R04 | Expose image identity | NOT RUN | — |
| BR-470-09-R05 | Expose build identity | NOT RUN | — |
| BR-470-09-R06 | Prevent secret leakage | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-09**. The next package may consume this package only when its declared prerequisite status is satisfied.
