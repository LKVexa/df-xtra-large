# QUORUM BR-470-07 — Mailbox device
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Mailbox device** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.h::BR_MEMORY_BYTES/BR_FRAGMENT_BYTES`

### Atomic requirements
- **BR-470-07-R01:** Support host-to-guest messages
- **BR-470-07-R02:** Support guest-to-host messages
- **BR-470-07-R03:** Bound packets
- **BR-470-07-R04:** Define explicit ownership
- **BR-470-07-R05:** Prohibit raw pointer exchange
...
Define the invariants for **Mailbox device** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-07-R01 — Support host-to-guest messages.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-470-07-R02 — Support guest-to-host messages.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-470-07-R03 — Bound packets.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-07-R04 — Define explicit ownership.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-470-07-R05 — Prohibit raw pointer exchange.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-470-07_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-07_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-07_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-07_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-07-R01 | Support host-to-guest messages | NOT RUN | — |
| BR-470-07-R02 | Support guest-to-host messages | NOT RUN | — |
| BR-470-07-R03 | Bound packets | NOT RUN | — |
| BR-470-07-R04 | Define explicit ownership | NOT RUN | — |
| BR-470-07-R05 | Prohibit raw pointer exchange | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-07**. The next package may consume this package only when its declared prerequisite status is satisfied.
