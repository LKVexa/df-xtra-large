# QUORUM BR-470-01 — Device ABI

**Release sequence:** BOTTLE ROCKET 4.7.0 — Virtual Device, I/O & Service Architecture  
**Purpose:** Provide controlled interaction with the world without compromising VM isolation.  
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Device ABI** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-01-R01:** Define device ID
- **BR-470-01-R02:** Define device version
- **BR-470-01-R03:** Define required capability
- **BR-470-01-R04:** Define operation number
- **BR-470-01-R05:** Define input buffer
- **BR-470-01-R06:** Define output buffer
- **BR-470-01-R07:** Define status
- **BR-470-01-R08:** Declare deterministic or non-deterministic behavior
...
Define the invariants for **Device ABI** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-01-R01 — Define device ID.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
2. **BR-470-01-R02 — Define device version.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
3. **BR-470-01-R03 — Define required capability.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
4. **BR-470-01-R04 — Define operation number.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
5. **BR-470-01-R05 — Define input buffer.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
6. **BR-470-01-R06 — Define output buffer.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
7. **BR-470-01-R07 — Define status.** Specify the contract precisely, encode it in machine-readable form where possible, and bind tests to the specification.
8. **BR-470-01-R08 — Declare deterministic or non-deterministic behavior.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-470-01_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-01_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-01_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-01_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-01-R01 | Define device ID | NOT RUN | — |
| BR-470-01-R02 | Define device version | NOT RUN | — |
| BR-470-01-R03 | Define required capability | NOT RUN | — |
| BR-470-01-R04 | Define operation number | NOT RUN | — |
| BR-470-01-R05 | Define input buffer | NOT RUN | — |
| BR-470-01-R06 | Define output buffer | NOT RUN | — |
| BR-470-01-R07 | Define status | NOT RUN | — |
| BR-470-01-R08 | Declare deterministic or non-deterministic behavior | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-01**. The next package may consume this package only when its declared prerequisite status is satisfied.
