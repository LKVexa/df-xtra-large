# QUORUM BR-470-10 — Network policy
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Network policy** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-10-R01:** No network device by default
- **BR-470-10-R02:** Qualify optional network extension separately
- **BR-470-10-R03:** Capability-gate networking
- **BR-470-10-R04:** Bound packets
- **BR-470-10-R05:** Use explicit protocol adapter
- **BR-470-10-R06:** Prohibit implicit host networking
...
Define the invariants for **Network policy** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-10-R01 — No network device by default.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-10-R02 — Qualify optional network extension separately.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-470-10-R03 — Capability-gate networking.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-10-R04 — Bound packets.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-10-R05 — Use explicit protocol adapter.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
6. **BR-470-10-R06 — Prohibit implicit host networking.** Create a fail-closed enforcement path, a negative regression test proving the forbidden path cannot succeed, and production-build checks preventing reintroduction.
...
### Q7 — Evidence package
Generate:
- `BR-470-10_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-10_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-10_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-10_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-10-R01 | No network device by default | NOT RUN | — |
| BR-470-10-R02 | Qualify optional network extension separately | NOT RUN | — |
| BR-470-10-R03 | Capability-gate networking | NOT RUN | — |
| BR-470-10-R04 | Bound packets | NOT RUN | — |
| BR-470-10-R05 | Use explicit protocol adapter | NOT RUN | — |
| BR-470-10-R06 | Prohibit implicit host networking | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-10**. The next package may consume this package only when its declared prerequisite status is satisfied.
