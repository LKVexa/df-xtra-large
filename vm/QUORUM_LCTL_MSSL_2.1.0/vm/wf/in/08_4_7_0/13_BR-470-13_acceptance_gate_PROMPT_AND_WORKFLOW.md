# QUORUM BR-470-13 — Acceptance gate
...
### Atomic requirements
- **BR-470-13-R01:** Guest cannot interact with host outside device ABI
- **BR-470-13-R02:** Unauthorized device use traps
- **BR-470-13-R03:** Device inputs are bounds-safe
- **BR-470-13-R04:** Replay substitutes non-deterministic inputs correctly
...
### Q2 — Implementation
1. **BR-470-13-R01 — Guest cannot interact with host outside device ABI.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-13-R02 — Unauthorized device use traps.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-470-13-R03 — Device inputs are bounds-safe.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-13-R04 — Replay substitutes non-deterministic inputs correctly.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-470-13_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-13_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-13_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-13_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-13-R01 | Guest cannot interact with host outside device ABI | NOT RUN | — |
| BR-470-13-R02 | Unauthorized device use traps | NOT RUN | — |
| BR-470-13-R03 | Device inputs are bounds-safe | NOT RUN | — |
| BR-470-13-R04 | Replay substitutes non-deterministic inputs correctly | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-13**. The next package may consume this package only when its declared prerequisite status is satisfied.
