# QUORUM BR-470-02 — Console device
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Console device** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-02-R01:** Implement input
- **BR-470-02-R02:** Implement output
- **BR-470-02-R03:** Implement flush
- **BR-470-02-R04:** Bound message length
- **BR-470-02-R05:** Require capability
...
Define the invariants for **Console device** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-02-R01 — Implement input.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-470-02-R02 — Implement output.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-470-02-R03 — Implement flush.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-470-02-R04 — Bound message length.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-02-R05 — Require capability.** Convert this into a release gate backed by direct evidence; any missing or stale proof must resolve to PARTIAL or BLOCKED rather than PASS.
...
### Q7 — Evidence package
Generate:
- `BR-470-02_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-02_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-02_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-02_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-02-R01 | Implement input | NOT RUN | — |
| BR-470-02-R02 | Implement output | NOT RUN | — |
| BR-470-02-R03 | Implement flush | NOT RUN | — |
| BR-470-02-R04 | Bound message length | NOT RUN | — |
| BR-470-02-R05 | Require capability | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-02**. The next package may consume this package only when its declared prerequisite status is satisfied.
