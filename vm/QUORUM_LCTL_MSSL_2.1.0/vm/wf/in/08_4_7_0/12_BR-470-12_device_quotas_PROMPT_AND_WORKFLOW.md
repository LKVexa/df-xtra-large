# QUORUM BR-470-12 — Device quotas
...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **Device quotas** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
### Atomic requirements
- **BR-470-12-R01:** Limit calls per execution
- **BR-470-12-R02:** Limit bytes transferred
- **BR-470-12-R03:** Limit storage writes
- **BR-470-12-R04:** Limit entropy requests
- **BR-470-12-R05:** Limit mailbox depth
...
Define the invariants for **Device quotas** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-12-R01 — Limit calls per execution.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-12-R02 — Limit bytes transferred.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-470-12-R03 — Limit storage writes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-12-R04 — Limit entropy requests.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-12-R05 — Limit mailbox depth.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-470-12_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-12_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-12_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-12_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-12-R01 | Limit calls per execution | NOT RUN | — |
| BR-470-12-R02 | Limit bytes transferred | NOT RUN | — |
| BR-470-12-R03 | Limit storage writes | NOT RUN | — |
| BR-470-12-R04 | Limit entropy requests | NOT RUN | — |
| BR-470-12-R05 | Limit mailbox depth | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-12**. The next package may consume this package only when its declared prerequisite status is satisfied.
