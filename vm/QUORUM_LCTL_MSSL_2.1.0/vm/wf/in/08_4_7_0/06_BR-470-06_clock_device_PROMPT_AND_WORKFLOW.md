...
### Atomic requirements
- **BR-470-06-R01:** Provide monotonic time
- **BR-470-06-R02:** Provide optional wall time
- **BR-470-06-R03:** Provide deterministic test clock
- **BR-470-06-R04:** Capability-gate clock usage
...
Define the invariants for **Clock device** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-06-R01 — Provide monotonic time.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-470-06-R02 — Provide optional wall time.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-470-06-R03 — Provide deterministic test clock.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
4. **BR-470-06-R04 — Capability-gate clock usage.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-470-06_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-06_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-06_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-06_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-06-R01 | Provide monotonic time | NOT RUN | — |
| BR-470-06-R02 | Provide optional wall time | NOT RUN | — |
| BR-470-06-R03 | Provide deterministic test clock | NOT RUN | — |
| BR-470-06-R04 | Capability-gate clock usage | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-06**. The next package may consume this package only when its declared prerequisite status is satisfied.
