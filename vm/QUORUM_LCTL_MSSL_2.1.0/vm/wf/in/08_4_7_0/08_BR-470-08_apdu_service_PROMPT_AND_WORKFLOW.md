...
Using QUORUM, upgrade the attached BOTTLE ROCKET 4.0.0 COLUMNED LCTL repository to satisfy **APDU service** as an evidence-backed step toward BOTTLE ROCKET 5.0.0 FULLY OPERATIONAL VM. Work directly against the repository; do not replace executable behavior with prose, mocks, placeholder PASS files, or unverifiable claims. Preserve working behavior unless this package explicitly supersedes it.
...
- `src/brvm.h::br_apdu_command/br_apdu_status`

### Atomic requirements
- **BR-470-08-R01:** Harden APDU parser
- **BR-470-08-R02:** Enforce strict lengths
- **BR-470-08-R03:** Enforce strict command classes
- **BR-470-08-R04:** Authenticate privileged commands
- **BR-470-08-R05:** Separate production and development commands
- **BR-470-08-R06:** Build fuzz corpus
...
Define the invariants for **APDU service** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-08-R01 — Harden APDU parser.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
2. **BR-470-08-R02 — Enforce strict lengths.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
3. **BR-470-08-R03 — Enforce strict command classes.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
4. **BR-470-08-R04 — Authenticate privileged commands.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
5. **BR-470-08-R05 — Separate production and development commands.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
6. **BR-470-08-R06 — Build fuzz corpus.** Add deterministic positive and negative tests, adversarial cases, bounded resource assertions, and machine-readable pass/fail evidence.
...
### Q7 — Evidence package
Generate:
- `BR-470-08_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-08_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-08_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-08_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-08-R01 | Harden APDU parser | NOT RUN | — |
| BR-470-08-R02 | Enforce strict lengths | NOT RUN | — |
| BR-470-08-R03 | Enforce strict command classes | NOT RUN | — |
| BR-470-08-R04 | Authenticate privileged commands | NOT RUN | — |
| BR-470-08-R05 | Separate production and development commands | NOT RUN | — |
| BR-470-08-R06 | Build fuzz corpus | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-08**. The next package may consume this package only when its declared prerequisite status is satisfied.
