# QUORUM BR-470-03 — Persistent block device
...
### Atomic requirements
- **BR-470-03-R01:** Implement read
- **BR-470-03-R02:** Implement write
- **BR-470-03-R03:** Implement sync
- **BR-470-03-R04:** Implement capacity query
- **BR-470-03-R05:** Enforce bounds
...
Define the invariants for **Persistent block device** before editing code. At minimum specify: accepted inputs, rejected inputs, state transitions, trust boundary, resource ceiling, deterministic/replay behavior, compatibility/version behavior, and the exact condition that produces OPERATIONAL status.

### Q2 — Implementation
1. **BR-470-03-R01 — Implement read.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
2. **BR-470-03-R02 — Implement write.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
3. **BR-470-03-R03 — Implement sync.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
4. **BR-470-03-R04 — Implement capacity query.** Implement the smallest deterministic mechanism that satisfies the requirement, expose explicit interfaces, and prove behavior with unit, integration, failure-path, and replay tests.
5. **BR-470-03-R05 — Enforce bounds.** Implement and verify this requirement with explicit invariants, negative tests, deterministic replay, resource bounds, and evidence artifacts.
...
### Q7 — Evidence package
Generate:
- `BR-470-03_requirements.json` — status and evidence hash for every atomic requirement.
- `BR-470-03_tests.log` — exact commands, exit codes, and salient output.
- `BR-470-03_audit.md` — human-readable before/after findings and unresolved blockers.
- `BR-470-03_manifest.sha256` — hashes of every file changed or generated by this work package.
...
|---|---|---|---|
| BR-470-03-R01 | Implement read | NOT RUN | — |
| BR-470-03-R02 | Implement write | NOT RUN | — |
| BR-470-03-R03 | Implement sync | NOT RUN | — |
| BR-470-03-R04 | Implement capacity query | NOT RUN | — |
| BR-470-03-R05 | Enforce bounds | NOT RUN | — |

## Required output
Return a modified repository or patch set, the complete evidence package, exact reproduction commands, a concise change log, unresolved blockers, and the final gate decision for **BR-470-03**. The next package may consume this package only when its declared prerequisite status is satisfied.
